...
for i in `grep -rl humanitas * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;humanitas;${THEME_NAME};" $i $i; done
for i in `grep -rl Humanitas * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;Humanitas;${THEME_NAME};" $i $i; done
for i in `grep -rl air * --exclude-dir=node_modules 2> /dev/null`; do LC_ALL=C sed -i '' -e "s;humanitas;${THEME_NAME};" $i $i; done
