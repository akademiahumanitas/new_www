...
function render_acf_block( $block, $content = '', $is_preview = false, $post_id = 0 ) {
  ...
  $block_path = get_theme_file_path( "template-parts/blocks/{$block_slug}/index.php" );
  ...
  // if in block [data][show_in_secondary_nav] is set to true, use block [data] [secondary_nav_title] ad block['id']
  if(isset($block['data']['show_in_secondary_nav']) && $block['data']['show_in_secondary_nav']) {
    // check if  $block['data']['secondary_nav_title']  is set 
    if(isset($block['data']['secondary_nav_title']) && $block['data']['secondary_nav_title'] != '') {
      $block['id'] = sanitize_title($block['data']['secondary_nav_title']);
    }
  }
...
function load_acf_block( $block_path, $cache = false, $block = array(), $is_preview = false, $post_id = 0 ) {
  ...
  if($block['data']['is_example'] == true) {
      $block_name = str_replace('acf/', '', $block['name']);
      $screenshot_url = 'template-parts/blocks/' . $block_name . '/screenshot.jpg';
      $screenshot = get_theme_file_uri( $screenshot_url );
      return '<img src="' . $screenshot . '" alt="Example block" style="width: 100%; height: auto;">';
  }
