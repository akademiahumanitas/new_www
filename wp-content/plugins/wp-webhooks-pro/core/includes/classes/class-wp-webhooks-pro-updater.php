	...
	class Ironikus_Webhook_Pro_Updater {
		...
		public function show_update_notification( $file, $plugin ) {
			...
			printf(
				...
				esc_html__( 'There is a new version of %1$s available.', 'wp-webhooks' ),
			...
			if ( ! current_user_can( 'update_plugins' ) ) {
				...
				printf(
					...
					__( '%1$sView version %2$s details%3$s.', 'wp-webhooks' ),
				...
				echo ' ';
				printf(
					__( '%1$sView version %2$s details%3$s or %4$supdate now%5$s.', 'wp-webhooks' ),
					'<a target="_blank" class="thickbox open-plugin-details-modal" href="' . esc_url( $changelog_link ) . '">',
					esc_html( $update_cache->response[ $this->name ]->new_version ),
					'</a>',
				...
				printf(
					' %1$s%2$s%3$s',
					'<a target="_blank" class="update-link" href="' . esc_url( wp_nonce_url( $update_link, 'upgrade-plugin_' . $file ) ) . '">',
					esc_html__( 'Update now.', 'wp-webhooks' ),
					'</a>'
				);
			}
		...
		public function show_changelog() {
			...
			if ( ! current_user_can( 'update_plugins' ) ) {
				wp_die( esc_html__( 'You do not have permission to install plugin updates', 'wp-webhooks' ), esc_html__( 'Error', 'wp-webhooks' ), array( 'response' => 403 ) );
			}
