...
class WP_Webhooks_Pro_Fields{
	...
	public function execute(){
		...
		//Register predefined query field callbacks
		...
		add_filter( 'wpwhpro/fields/get_query_items/network_sites', array( $this, 'filter_query_network_site_items' ), 20, 3 );
		...
		add_filter( 'wpwhpro/fields/get_query_items/flows', array( $this, 'filter_query_flow_items' ), 20, 3 );
		add_filter( 'wpwhpro/fields/get_query_items/authentications', array( $this, 'filter_query_authentication_items' ), 20, 3 );
		add_filter( 'wpwhpro/fields/get_query_items/countries', array( $this, 'filter_query_country_items' ), 20, 3 );
		add_filter( 'wpwhpro/fields/get_query_items/timezones', array( $this, 'filter_query_timezone_items' ), 20, 3 );
	...
	public function get_field( $args = array(), $type = 'input' ){
		...
		switch( $type ){
			...
			case 'input':
				...
				break;
			case 'checkbox':
				$field = $this->prepare_checkbox_field( $args );
				break;
		}

		return apply_filters( 'wpwhpro/fields/get_field', $field, $type, $args );
	}
	 ...
	 */
	 ...
	 private function prepare_input_field( $args = array() ){
		...
		$field = array_merge( $default_args, $field_defaults, $args );
	 ...
	 private function prepare_checkbox_field( $args = array() ){
		...
		return apply_filters( 'wpwhpro/fields/prepare_checkbox_field', $field, $args, $field_defaults );
	 }
	...
	public function filter_query_user_items( $entries, $query_args, $args ){
	 ...
	 /**
	  * Filter items using the WP_Site_Query. 
	  * This filter support all argument of the WP_Site_Query class
	  ...
	  * @return array
	  */
	public function filter_query_network_site_items( $entries, $query_args, $args ){

		//Only run if a multisite is given:
		if( ! is_multisite() ){
			return $entries;
		}
		...
		if( isset( $args['paged'] ) ){
			$query_args['offset'] = $entries['per_page'] * ( intval( $args['paged'] ) -1 );
		}
		...
		if( isset( $args['selected'] ) && ! empty( $args['selected'] ) && is_array( $args['selected'] ) ){
			...
			//since we work with selected values, make sure we force the number
			...
			$query_args['site__in'] = $selected_items;
		}

		$query_args = array_merge( $default_args, $query_args );	

		$sites_query = get_sites( $query_args );

		if( ! empty( $sites_query ) ){
			foreach( $sites_query as $site ){
				$entries['items'][ $site->blog_id ] = array(
					'value' => $site->blog_id,
					'label' => '#' . $site->blog_id . ' - ' . $site->blogname,
				);
			}
		}
		...
		if( $is_selected ){
			...
			$site_count = wp_count_sites();

			if( isset( $site_count['all'] ) ){
				$entries['total'] = intval( $site_count['all'] );
			}
		}
	...
	public function filter_query_post_type_items( $entries, $query_args, $args ){
		...
		if( isset( $args['s'] ) && ! empty( $args['s'] ) ){
			foreach( $entries['items'] as $item_key => $item_data ){
				...
				if(
					stripos( $item_data['value'], $args['s'] ) === false
					&& stripos( $item_data['label'], $args['s'] ) === false
				){
	...
	/**
	  ...
	  * @return array
	  */
	  public function filter_query_flow_items( $entries, $query_args, $args ){
		...
		if ( is_array( $flows ) && ! empty( $flows ) ) {
			foreach ( $flows as $flow ) {
				...
				if( isset( $flow->id ) ){
					$flow_id = intval( $flow->id );

					$entries['items'][ $flow_id ] = array(
						'value' => $flow_id,
						'label' => '#' . $flow_id . ' - ' . esc_html( $flow->flow_title ),
					);
				}
	...
	/**
	  * Filter items using the WPWHPRO()->auth->template_query() function. 
	  * This filter support all argument of the WPWHPRO()->auth->template_query() function
	  *
	  * @param array $entries - the available and found items
	  * @param array $query_args - The arguments defined within the settings query
	  * @param array $args - custom arguments that are used for the filter (e.g. paged, s - for a string search)
	  * @return array
	  */
	  public function filter_query_authentication_items( $entries, $query_args, $args ){
		...
		if( isset( $args['selected'] ) && ! empty( $args['selected'] ) && is_array( $args['selected'] ) ){
			...
			$query_args['items__in'] = $selected_items;
		}

		$query_args = array_merge( $default_args, $query_args );

			
		$auth_query = WPWHPRO()->auth->template_query( $query_args );

		if( ! empty( $auth_query ) && ! empty( $auth_query['items'] ) ){	

			foreach( $auth_query['items'] as $item ){
			...
			if( isset( $auth_query['count_total'] ) ){
				$entries['total'] = intval( $auth_query->count_total );
			}
		}
	...
	/**
	  * Filter the items based on the available list of countries
	  *
	  * @since 6.0.1
	  *
	  * @param array $entries - the available and found items
	  ...
	  * @return array
	  */
	  public function filter_query_country_items( $entries, $query_args, $args ){
		...
		if ( is_array( $countries ) && ! empty( $countries ) ) {
			foreach ( $countries as $country_code => $country_name ) {
				...
				$entries['items'][ $country_code ] = array(
					'value' => $country_code,
	...
	/**
	  * Filter the items based on the available list of timezones
	  *
	  * @since 6.0.4
	  *
	  * @param array $entries - the available and found items
	  ...
	  * @return array
	  */
	  public function filter_query_timezone_items( $entries, $query_args, $args ){
		...
		$timezones = WPWHPRO()->helpers->get_timezones_array();	

		if ( is_array( $timezones ) && ! empty( $timezones ) ) {
			foreach ( $timezones as $timezone ) {

				$entries['items'][ $timezone['value'] ] = array(
					'value' => $timezone['value'],
					'label' => esc_html( $timezone['label'] ),
				);
