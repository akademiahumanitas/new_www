...
class WP_Webhooks_Pro_Webhook {
	...
	/**
	 * Execute Webhooks related hooks and logic to get 
	 * everything running
	 *
	 * @since 6.0
	 * @return void
	...
	public function execute(){
		...
		add_action( 'wpwh_schedule_action_execution_callback', array( $this, 'wpwh_schedule_action_execution_callback' ), 10, 2 );
		...
		switch( $type ){
			case 'action':
				...
				if( isset( $args['integration'] ) && ! empty( $args['integration'] ) ){
					$data['integration'] = sanitize_title( $args['integration'] );
				}
	...
	public function update( $key, $type, $group = '', $args = array() ){
		...
		$check = false;
		if( ! empty( $data ) && is_array( $data ) && is_array( $args ) ){
	...
	public function initialize_default_webhook(){
		...
		WPWHPRO()->helpers->log_issue( __( "The function WPWHPRO()->webhook->initialize_default_webhook() is deprecated since 5.0. Webhook actions are now available for each endpoint separately.", 'admin-debug-feature' ) );
		return;
	...
	public function reset_wpwhpro(){
		...
		$shared_tables = get_option( 'wpwhpro_sync_network_tables' );
		...
		//Reset previous versions option
		delete_option( WPWHPRO()->settings->get_wpwh_previous_versions_key() );
		...
		if( ! $shared_tables || $shared_tables === 'no' ){
		...
		//uninstall all integrations only if no multisite is given
		if( ! is_multisite() ){
			if( $is_locally_loaded !== 'yes' ){
				WPWHPRO()->integrations->maybe_uninstall_integrations();
			}
		}
	...
	public function get_incoming_action( $response_body = false ){
		...
		if( empty( $action ) ){
			...
			if( empty( $action ) ){
				WPWHPRO()->helpers->log_issue( __( "The incoming webhook call did not contain any action argument.", 'admin-debug-feature' )  );
			}
		}
	...
	/**
	 * Validate the endpoint description attributes
	 * 
	 * Since 6.0.1, we accept as well the endpoint details
	 * for a smoother validation. Simply provide the details array
	 * within the $type variable
	 *
	 * @since 5.2.4
	 * @return array Validated attribute data
	 */
	public function validate_endpoint_description_args( $type = 'trigger', $data = array() ){
		$endpoint_details = null;

		//run the new 6.0.1 logic
		if( is_array( $type ) ){
			$endpoint_details = $type;
			$type = 'trigger'; //reset type
			$endpoint_name = '';
			$endpoint_slug = '';

			if( isset( $endpoint_details['name'] ) ){
				$endpoint_name = $endpoint_details['name'];
			}

			if( isset( $endpoint_details['trigger'] ) ){
				$type = 'trigger';
				$endpoint_slug = $endpoint_details['trigger'];
			} elseif( isset( $endpoint_details['action'] ) ){
				$type = 'action';
				$endpoint_slug = $endpoint_details['action'];
			}

			if( isset( $endpoint_details['description'] ) ){
				$data = $endpoint_details['description'];
			}

			$data['webhook_name'] = $endpoint_name;
			$data['webhook_slug'] = $endpoint_slug;
		}
		...
		//Add default attributes depending on the type
		switch( $type ){
			case 'trigger':
				$default_attributes['trigger_hooks'] = array(); //todo - remove this parameter
				break;
			case 'action':
		...
		$attributes = array_merge( $default_attributes, $data );

		return apply_filters( 'wpwhpro/webhooks/validate_endpoint_description_args', $attributes, $type, $data, $endpoint_details );
	}
	...
	/**
      ...
      */
	  public function schedule_action_execution( $timestamp, $action, $request ){
        $response = array(
			'success' => false,
			'msg' => __( 'The action was not scheduled due to an error.', 'wp-webhooks' ),
		);
        ...
        if( ! empty( $timestamp ) ){
            ...
            $response = WPWHPRO()->scheduler->schedule_single_action( array(
                ...
                'hook' => 'wpwh_schedule_action_execution_callback',
                'attributes' => $attributes,
            ) );

            //customize the response message
            if( $response['success'] ){
                $response['msg'] = __( 'The action was successfully scheduled for execution.', 'wp-webhooks' );
            }

        }

        return apply_filters( 'wpwhpro/http/schedule_action_execution', $response, $timestamp, $action, $request );
    }

	/**
      * Handles the callback of a scheduled Webhook action
      *
	  * @since 6.0
      * @param string $url The request URL
      ...
      * @return void
      */
	  public function wpwh_schedule_action_execution_callback( $action, $request = array() ){
		$response = array(
			'success' => false,
			'msg' => __( 'The scheduled action was executed, but nothing happened.', 'wp-webhooks' ),
		);

		//Maybe set GET values based on the given URL 
		if( isset( $request['query'] ) && is_array( $request['query'] ) ){
			$_GET = array_merge( $request['query'], $_GET ); //Prioritize current GET
		}

        $response = WPWHPRO()->integrations->execute_actions( $response, $action, $request );

        do_action( 'wpwhpro/http/wpwh_schedule_action_execution_callback', $response, $action, $request );
	...
	public function validate_incoming_data(){
		...
		$skip_scheduling = ( ! empty( $_REQUEST['skip_scheduling'] ) && $_REQUEST['skip_scheduling'] === 'yes' ) ? true : false;
		...
		if( WPWHPRO()->whitelist->is_active() ){
			if( ! WPWHPRO()->whitelist->is_valid_request() ){
				...
				status_header( 403 );
				$return['msg'] = __( 'You don\'t have permission to post data to this site since you are not on the whitelist.', 'wp-webhooks' );
		...
		if( is_array( $webhook ) && isset( $webhook['status'] ) ){
			if( $webhook['status'] === 'inactive' ){
				status_header( 403 );
				$return['msg'] = sprintf( __( 'Your current %s webhook action URL is deactivated. Please activate it first.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() );
				WPWHPRO()->webhook->echo_response_data( $return, true );
				exit;
			}
		}
		...
		if( ! empty( $webhook ) && is_array( $webhook ) ){
			if( isset( $webhook['api_key'] ) ){
				if( $webhook['api_key'] != $response_api_key ){
					status_header( 403 );
					$return['msg'] = sprintf( __( 'The given %s API Key is not valid, please enter a valid API key and try again.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() );
					WPWHPRO()->webhook->echo_response_data( $return, true );
					exit;
				}
			} else {
				status_header( 403 );
				$return['msg'] = sprintf( __( 'The given %s API Key is missing, please add it first.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() );
				WPWHPRO()->webhook->echo_response_data( $return, true );
				exit;
			}
			...
			status_header( 403 );
			$return['msg'] = sprintf( __( 'We could not locate a webhook action URL for your given data. Please make sure you set the URL (query) parameter &action=youraction', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() );
			WPWHPRO()->webhook->echo_response_data( $return, true );
			exit;
		}
		...
		if( $is_flow ){
			...
			$message = __( 'Flow action request received.', 'wp-webhooks' );
		} else {
			$message = __( 'Action request received.', 'wp-webhooks' );
		}
		...
		if( $response_auth_request ){
			$return_auth = array(
				'success' => true,
				'msg' => __( 'The authentication was successful', 'wp-webhooks' ),
				'domain' => home_url(),
			...
			foreach( $webhook['settings'] as $settings_name => $settings_data ){
				...
				if( $settings_name === 'wpwhpro_action_access_token' && ! empty( $settings_data ) ){
					...
					if( empty( $access_token ) || $access_token !== $settings_data ){
						status_header( 403 );
						$return['msg'] = __( 'Your configured access token for this webhook URL is not valid.', 'wp-webhooks' );
				...
				if( $settings_name === 'wpwhpro_action_accepted_methods' && ! empty( $settings_data ) ){
					...
					if( empty( $current_request_method ) || ! in_array( $current_request_method, $settings_data ) ){
						status_header( 403 );
						$return['msg'] = __( 'Your current request method is not allowed. Please use only the request methods you selected within the webhook action URL settings.', 'wp-webhooks' );
				...
				if( $settings_name === 'wpwhpro_action_action_whitelist' && ! empty( $settings_data ) ){
					if( ! in_array( $action, $settings_data ) ){
						status_header( 403 );
						$return['msg'] = __( 'You don\'t have permission to use this specific webhook action.', 'wp-webhooks' );
				...
				if( $settings_name === 'wpwhpro_action_authentication' && ! empty( $settings_data ) ){
					...
					if( 
						is_numeric( $settings_data ) 
						&& ! empty( WPWHPRO()->auth->get_template( intval( $settings_data ) ) )
					){
				...
				if( ! $skip_scheduling && $settings_name === 'wpwhpro_action_schedule' && ! empty( $settings_data ) && is_numeric( $settings_data ) ){
					$schedule_action = intval( $settings_data );

					$schedule_timestamp = time() + $schedule_action;
				}
		...
		$default_return_data = array(
            'success' => false,
			'action' => $action,
			'msg' => __( "It looks like your current webhook call has no action argument defined, it is deactivated or it does not have any action function.", 'wp-webhooks' ),
        );
		...
		if( apply_filters( 'wpwhpro/webhooks/validate_webhook_action', true, $action, $response_ident_value, $response_api_key ) ){
			...
			if( $schedule_action ){
				$return_data = $this->schedule_action_execution( $schedule_timestamp, $action, WPWHPRO()->http->get_current_request() );
			} else {
				//since 4.2.0
			...
			$default_return_data['msg'] = __( "The webhook action was prevented from execution due to the wpwhpro/webhooks/validate_webhook_action filter returning false.", 'wp-webhooks' );
	...
	public function execute_receivable_trigger(){
		...
		if( empty( $trigger ) ){
			status_header( 403 );
			$return['msg'] = __( 'We could not find a valid trigger for your given URL.', 'wp-webhooks' );
			WPWHPRO()->webhook->echo_response_data( $return, true );
			exit;
		}
		...
		if( ! isset( $trigger['secret'] ) || empty( $trigger['secret'] ) || ! is_string( $trigger['secret'] ) ){
			status_header( 403 );
			$return['msg'] = __( 'We could not verify your receivable trigger URL.', 'wp-webhooks' );
			WPWHPRO()->webhook->echo_response_data( $return, true );
			exit;
		}
		...
		if( $receivable_trigger_signature !== $real_signature ){
			status_header( 403 );
			$return['msg'] = __( 'Your given webhook trigger URL is not valid.', 'wp-webhooks' );
			WPWHPRO()->webhook->echo_response_data( $return, true );
			exit;
		}
		...
		if( empty( $trigger_group ) || ! isset( $trigger_group['receivable_url'] ) || $trigger_group['receivable_url'] !== true ){
			status_header( 403 );
			$return['msg'] = __( 'Your given webhook trigger has no permission to do this.', 'wp-webhooks' );
			WPWHPRO()->webhook->echo_response_data( $return, true );
			exit;
		}
		...
		$default_return_data = array(
            'success' => false,
			'msg' => __( "The receivable trigger URL has been successfully executed.", 'wp-webhooks' ),
        );
	...
	public function post_to_webhook( $webhook, $data, $args = array(), $skip_validation = false ){
		...
		if( is_array($webhook) && isset( $webhook['settings'] ) && ! empty( $webhook['settings'] ) ) {
			...
			foreach ( $webhook['settings'] as $settings_name => $settings_data ) {
				...
				if( $settings_name === 'wpwhpro_trigger_authentication' && ! empty( $settings_data ) ){
					...
					if( is_numeric( $settings_data ) ){
						$template = WPWHPRO()->auth->get_template( $settings_data );
					...
					switch( $settings_data ){
						...
						case 'xml':
							if( extension_loaded('simplexml') ){
								...
								$response['msg'] = __( 'SimpleXML is not activated on your server. Please activate it first or switch the content type of your webhook.', 'wp-webhooks' );
				...
				if( $settings_name === 'wpwhpro_trigger_wait_response' && (integer) $settings_data === 1 ){
			...
			foreach( $webhook['settings'] as $settings_name => $settings_data ){
				...
				if( $settings_name === 'wpwhpro_user_must_be_logged_in' && (integer) $settings_data === 1 ){
					if( ! is_user_logged_in() ){
						$response['msg'] = __( 'Trigger not sent because the settings did not match.', 'wp-webhooks' );
						$response['is_valid'] = false;
					}
				}
				...
				if( $settings_name === 'wpwhpro_trigger_single_instance_execution' && (integer) $settings_data === 1 ){
					...
					if( is_array( $all_processed_triggers ) && ! empty( $all_processed_triggers ) && isset( $all_processed_triggers[ $webhook_name . '_' . $webhook_url_name ] ) ){
						$response['msg'] = __( 'This was a duplicate request as the Single Instance Execution was set for the webhook.', 'wp-webhooks' );
						$response['is_valid'] = false;
				...
				//Schedule a trigger
				if( $settings_name === 'wpwhpro_trigger_schedule' && ! empty( $settings_data ) && is_numeric( $settings_data ) ){
					$schedule_trigger = intval( $settings_data );

					$schedule_timestamp = time() + $schedule_trigger;
				}
		...
		if( isset( $webhook['status'] ) && ! $skip_validation ){
			if( $webhook['status'] === 'inactive' ){
				$response['msg'] = __( 'The following webhook trigger url is deactivated. Please activate it first.', 'wp-webhooks' );
				$response['is_valid'] = false;
			}
		}
		...
		if( $is_flow ){	
			...
			if( $flow_blocked_trigger !== $webhook_name ){
				...
				if( ! empty( $schedule_trigger ) ){
					$response = WPWHPRO()->flows->schedule_process_flow( $schedule_timestamp, $flow_id, array(
						'payload' => $data,
					) );
				} else {
				...
				$response = array(
					'success' => false,
					'msg' => __( 'The flow got canceled to avoid loops.', 'wp-webhooks' )
				);
			}
			...
			if( ! empty( $schedule_trigger ) ){
				$response = WPWHPRO()->http->schedule_http_request( $schedule_timestamp, $url, $http_args );
			} else {
		...
		if( $is_flow ){
			...
			if( ! empty( $schedule_trigger ) ){
				$message = sprintf( __( 'The Flow was scheduled to execute on %s!', 'wp-webhooks' ), WPWHPRO()->helpers->get_formatted_date( $schedule_timestamp, 'F j, Y, g:i a' ) );
			} else {
				$message = __( 'Flow successfully fired!', 'wp-webhooks' );
			}
			
		} else {

			if( ! empty( $schedule_trigger ) ){
				$message = sprintf( __( 'The trigger was scheduled to execute on %s!', 'wp-webhooks' ), WPWHPRO()->helpers->get_formatted_date( $schedule_timestamp, 'F j, Y, g:i a' ) );
			} else {
				$message = __( 'Trigger successfully sent!', 'wp-webhooks' );
			}
