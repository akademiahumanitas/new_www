...
/**
 * WP_Webhooks_Pro_WP_List_Table Class
 *
 ...
 * array(
 * 	'labels' => array(
 * 		'singular' => __( 'post', 'wp-webhooks' ),
 * 		'plural' => __( 'posts', 'wp-webhooks' ),
 * 		'search_placeholder' => '',
 * 	),
 * 	'settings' => array(
 * 		'per_page' => 20,
 ...
 */
class WP_Webhooks_Pro_WP_List_Table extends WP_List_Table {
...
<p class="search-box">
	...
	<input type="search" class="wpwh-form-input" id="<?php echo esc_attr( $input_id ); ?>" name="s" value="<?php _admin_search_query(); ?>" placeholder="<?php echo ( isset( $this->labels['search_placeholder'] ) ) ? esc_html( $this->labels['search_placeholder'] ) : ''; ?>" />
...
<div class="wpwh-list-table-wrapper">
	<form class="wpwh-table-form" method="get" action="">
		...
		<?php if( isset( $this->settings['show_search'] ) && ! empty( $this->settings['show_search'] ) ) : ?>
			<?php $this->search_box( sprintf( __( 'Search %s', 'wp-webhooks' ), $this->labels['plural'] ), 'wpwh-form-search' ); ?>
		<?php endif; ?>
		<?php parent::display(); ?>
	</form>
</div>
