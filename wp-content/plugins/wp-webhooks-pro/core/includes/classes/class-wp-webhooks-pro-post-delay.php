...
class WP_Webhooks_Pro_Post_Delay {
    ...
    public function launch_trigger( $trigger ){
        ...
        if( is_array( $trigger ) && isset( $trigger['callback'] ) && isset( $trigger['arguments'] ) && is_array( $trigger['arguments'] ) ){
            return call_user_func_array( $trigger['callback'], $trigger['arguments'] );
        } else {
            return null;
        }
	...
	public function add_post_delayed_trigger( $callback, $arguments = array(), $options = '' ){
        ...
        //only allow customization on an endpoint basis if the logic is turned on
        if( isset( $options['delay'] ) && $is_active ){
            $is_active = ( $options['delay'] ) ? true : false;
        }

        //Terminate the trigger immediately if the logic is deactivated
        if( ! $is_active ){
            return $this->launch_trigger( array(
                'callback' => $callback,
                'arguments' => $arguments,
                'options' => $options,
            ) );
