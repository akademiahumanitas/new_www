...
class WP_Webhooks_Pro_Data_Mapping {
	...
	public function ironikus_load_data_mapping_data(){
		...
		if( ! empty( $data_mapping_id ) && is_numeric( $data_mapping_id ) ){
		    ...
		    if( ! empty( $check ) ){
		        ...
		        $response['text'] 	 = array(
					'add_button_text' => __( 'Add Row', 'wp-webhooks' ),
					'import_button_text' => __( 'Import', 'wp-webhooks' ),
					'export_button_text' => __( 'Export', 'wp-webhooks' ),
					'add_first_row_text' => __( 'Add a row to get started!', 'wp-webhooks' ),
				);
				$response['data'] = $check;
	...
	public function apply_data_mapping_preview_template( $return, $mapping_template, $mapping_type ){
	 ...
	 /**
	 * Get the customized list class for the data mapping
	 *
	 * @since 6.1.0
	 *
	 * @return WP_Webhooks_Pro_WP_List_Table
	 */
	public function get_data_mapping_lists_table_class(){

		$args = array(
			'labels' => array(
				'singular' => __( 'data mapping', 'wp-webhooks' ),
				'plural' => __( 'data mappings', 'wp-webhooks' ),
				'search_placeholder' => __( 'Name...', 'wp-webhooks' ),
			),
			'columns' => array(
				'id' => array(
					'label' => __( 'Template ID', 'wp-webhooks' ),
					'callback' => array( $this, 'data_mapping_lists_cb_id' ),
					'actions_callback' => array( $this, 'data_mapping_lists_cb_id_actions' ),
					'sortable' => 'ASC',
				),
				'name' => array(
					'label' => __( 'Template name', 'wp-webhooks' ),
					'callback' => array( $this, 'data_mapping_lists_cb_name' ),
					'sortable' => 'ASC',
				),
				'date' => array(
					'label' => __( 'Date & Time', 'wp-webhooks' ),
					'callback' => array( $this, 'data_mapping_lists_cb_date' ),
					'sortable' => 'ASC',
				),
			),
			'settings' => array(
			...
			'item_filter' => array( $this, 'data_mapping_lists_filter_items' ),
		);
	 ...
	 * @return string
	 */
	public function data_mapping_lists_cb_id( $item, $column_name, $column ){
		$content = '';
		$current_url = WPWHPRO()->helpers->get_current_url( false, true );
		$title = '#' . $item->id;
		$edit_link = WPWHPRO()->helpers->built_url( $current_url, array_merge( $_GET, array( 'wpwhpro_data_mapping' => $item->id, ) ) );
	 ...
	 * 
	 * TODO: Make sure we switch data mapping 
	 * templates as well to the new lists logic
	 *
	 * @since 6.1.0
	 * @param object $item
	 * @param string $column_name
	 ...
	 * @return array
	 */
	public function data_mapping_lists_cb_id_actions( $item, $column_name, $primary, $column ){

		$details_title = __( 'Edit', 'wp-webhooks' );
		...
		$actions = array(
			'details' => WPWHPRO()->helpers->create_link( 
				...
				$details_title,
				array(
					'class' => 'wpwh-dm-view-template-btn',
					'title' => $details_title,
					'data-wpwh-mapping-id' => $item->id,
				)
			),
			'delete' => WPWHPRO()->helpers->create_link( 
				...
				$delete_title,
				array(
					'class' => 'wpwh-dm-delete-template-btn text-error',
					'title' => $delete_title,
					'data-wpwh-mapping-id' => $item->id,
				)
			),
	 ...
	 * @return string
	 */
	public function data_mapping_lists_cb_name( $item, $column_name, $column ){
		$content = '';

		if( isset( $item->name ) ){

			$content = sanitize_title( $item->name );
	 ...
	 * @return string
	 */
	public function data_mapping_lists_cb_template( $item, $column_name, $column ){
		$content = '';
	 ...
	 * @return string
	 */
	public function data_mapping_lists_cb_date( $item, $column_name, $column ){
	 ...
	 * @return void
	 */
	public function data_mapping_lists_filter_items( $args ){
		$item_data = array(
			'items' => array(),
			'total' => 0,
		);

		$query_data = $this->data_mapping_query( $args );
	...
	public function maybe_setup_data_mapping_table(){
		...
		if( ! WPWHPRO()->sql->table_exists( $this->data_mapping_table_data['table_name'] ) ){
			...
			WPWHPRO()->sql->update_table_exists_cache( $this->data_mapping_table_data['table_name'], 'exists' );
		}
	 ...
	 */
	public function data_mapping_query( $args = array() ){
		...
		$core_sql = 'FROM {prefix}' . $this->data_mapping_table_data['table_name'];
		...
		if( $query_data['s'] !== '' ){
			$core_sql .= ' ( name LIKE \'%' . esc_sql( $query_data['s'] ) . '%\' )  AND';
		}
	...
	public function delete_table(){
		...
		WPWHPRO()->sql->update_table_exists_cache( $this->data_mapping_table_data['table_name'], 'purge' );
