  ...
  */
  class WP_Webhooks_Integrations_integrately_Triggers_integrately_webhook_received {
	...
	public function get_details(){
		...
		$parameter = array(
			'custom_construct' => array( 'short_description' => __( '(Mixed) The data that was sent along with the HTTP call that was made to the receivable URL from within Integrately.', 'wp-webhooks' ) ),
		);

		$description = array(
			'steps' => array(
				__( 'Within the settings of this webhook trigger, copy the receivable URL.', 'wp-webhooks' ),
				__( 'Head into Integrately and select the "Webhook / API" integration along with the "POST" method.', 'wp-webhooks' ),
				__( 'Place the receivable URL there and send data based on your requirements.', 'wp-webhooks' ),
			),
			'tipps' => array(
				__( 'To receive data on the receivable URL, please use the "Webhook / API" integration within Integrately.', 'wp-webhooks' ),
		...
		return array(
			...
			'name'			  => __( 'Integrately webhook request received', 'wp-webhooks' ),
			'sentence'			  => __( 'a Integrately webhook request was received', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => sprintf( __( 'This webhook fires as soon as a request was received from the "Webhook / API" integration of Integrately.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() ),
