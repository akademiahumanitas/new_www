...
if ( ! class_exists( 'WP_Webhooks_Integrations_lemon_squeezy_Triggers_lemon_subscription_created' ) ) :

 /**
  * Load the lemon_subscription_created trigger
  *
  * @since 6.1.5
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_lemon_squeezy_Triggers_lemon_subscription_created {
	...
	public function get_details(){
		...
		$description = array(
			'steps' => array(
				...
				__( 'Place the receivable URL there and select the subscription_created webhook event.', 'wp-webhooks' ),
		...
		return array(
			'trigger'		   => 'lemon_subscription_created',
			'name'			  => __( 'Subscription created', 'wp-webhooks' ),
			'sentence'			  => __( 'a subscription is created', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => sprintf( __( 'This webhook fires as soon as a subscription is created within Lemon Squeezy.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() ),
	...
	public function execute( $return_data, $response_body, $trigger_url_name ){
		...
		if( $trigger_url_name !== null ){
			$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'lemon_subscription_created', $trigger_url_name );
			if( ! empty( $webhooks ) ){
			...
			$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'lemon_subscription_created' );
		}
		...
		do_action( 'wpwhpro/webhooks/trigger_lemon_subscription_created', $return_data, $response_body, $trigger_url_name, $response_data_array );
		...
		$data = array (
			...
			array (
			  ...
			  array (
				...
				'ends_at' => NULL,
				'created_at' => '2023-05-02T15:02:27.000000Z',
				'updated_at' => '2023-05-02T15:02:30.000000Z',
				'test_mode' => true,
			  ),
			  'relationships' => 
			  array (
