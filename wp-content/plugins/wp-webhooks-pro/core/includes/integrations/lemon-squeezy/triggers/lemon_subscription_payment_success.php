...
if ( ! class_exists( 'WP_Webhooks_Integrations_lemon_squeezy_Triggers_lemon_subscription_payment_success' ) ) :

 /**
  * Load the lemon_subscription_payment_success trigger
  *
  * @since 6.1.5
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_lemon_squeezy_Triggers_lemon_subscription_payment_success {
	...
	public function get_details(){
		...
		$description = array(
			'steps' => array(
				...
				__( 'Place the receivable URL there and select the subscription_payment_success webhook event.', 'wp-webhooks' ),
		...
		return array(
			'trigger'		   => 'lemon_subscription_payment_success',
			'name'			  => __( 'Subscription payment successful', 'wp-webhooks' ),
			'sentence'			  => __( 'a subscription payment is successful', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => sprintf( __( 'This webhook fires as soon as a subscription payment is successful within Lemon Squeezy.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() ),
	...
	public function execute( $return_data, $response_body, $trigger_url_name ){
		...
		if( $trigger_url_name !== null ){
			$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'lemon_subscription_payment_success', $trigger_url_name );
			if( ! empty( $webhooks ) ){
			...
			$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'lemon_subscription_payment_success' );
		}
		...
		do_action( 'wpwhpro/webhooks/trigger_lemon_subscription_payment_success', $return_data, $response_body, $trigger_url_name, $response_data_array );
