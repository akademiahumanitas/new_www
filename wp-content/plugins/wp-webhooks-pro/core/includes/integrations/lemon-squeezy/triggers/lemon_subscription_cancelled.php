...
if ( ! class_exists( 'WP_Webhooks_Integrations_lemon_squeezy_Triggers_lemon_subscription_cancelled' ) ) :

 /**
  * Load the lemon_subscription_cancelled trigger
  *
  * @since 6.1.5
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_lemon_squeezy_Triggers_lemon_subscription_cancelled {
	...
	public function get_details(){
		...
		$description = array(
			'steps' => array(
				...
				__( 'Place the receivable URL there and select the subscription_cancelled webhook event.', 'wp-webhooks' ),
		...
		return array(
			'trigger'		   => 'lemon_subscription_cancelled',
			'name'			  => __( 'Subscription cancelled', 'wp-webhooks' ),
			'sentence'			  => __( 'a subscription is cancelled', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => sprintf( __( 'This webhook fires as soon as a subscription is cancelled within Lemon Squeezy.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() ),
	...
	public function execute( $return_data, $response_body, $trigger_url_name ){
		...
		if( $trigger_url_name !== null ){
			$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'lemon_subscription_cancelled', $trigger_url_name );
			if( ! empty( $webhooks ) ){
			...
			$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'lemon_subscription_cancelled' );
		}
		...
		do_action( 'wpwhpro/webhooks/trigger_lemon_subscription_cancelled', $return_data, $response_body, $trigger_url_name, $response_data_array );
		...
		$data = array (
			...
			array (
			  ...
			  array (
				...
				'ends_at' => NULL,
				'cancelled_at' => '2023-05-02T15:02:27.000000Z',
				'cancelled_at' => '2023-05-02T15:02:30.000000Z',
				'test_mode' => true,
			  ),
			  'relationships' => 
			  array (
