...
if ( ! class_exists( 'WP_Webhooks_Integrations_lemon_squeezy_Triggers_lemon_subscription_resumed' ) ) :

 /**
  * Load the lemon_subscription_resumed trigger
  *
  * @since 6.1.5
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_lemon_squeezy_Triggers_lemon_subscription_resumed {
	...
	public function get_details(){
		...
		$description = array(
			'steps' => array(
				...
				__( 'Place the receivable URL there and select the subscription_resumed webhook event.', 'wp-webhooks' ),
		...
		return array(
			'trigger'		   => 'lemon_subscription_resumed',
			'name'			  => __( 'Subscription resumed', 'wp-webhooks' ),
			'sentence'			  => __( 'a subscription is resumed', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => sprintf( __( 'This webhook fires as soon as a subscription is resumed within Lemon Squeezy.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() ),
	...
	public function execute( $return_data, $response_body, $trigger_url_name ){
		...
		if( $trigger_url_name !== null ){
			$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'lemon_subscription_resumed', $trigger_url_name );
			if( ! empty( $webhooks ) ){
			...
			$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'lemon_subscription_resumed' );
		}
		...
		do_action( 'wpwhpro/webhooks/trigger_lemon_subscription_resumed', $return_data, $response_body, $trigger_url_name, $response_data_array );
		...
		$data = array (
			...
			array (
			  ...
			  array (
				...
				'ends_at' => NULL,
				'resumed_at' => '2023-05-02T15:02:27.000000Z',
				'resumed_at' => '2023-05-02T15:02:30.000000Z',
				'test_mode' => true,
			  ),
			  'relationships' => 
			  array (
