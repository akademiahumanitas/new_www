...
if ( ! class_exists( 'WP_Webhooks_Integrations_wp_fusion_Actions_wpfs_remove_tags' ) ) :
	...
	public function get_details(){
			...
			$parameter = array(
				...
				'tags'	=> array( 'required' => true, 'short_description' => __( 'Add the tags you want to remove the user from. This argument accepts a comma-separated string, as well as a JSON construct.', 'wp-webhooks' ) ),
			...
			ob_start();
		?>
<?php echo __( "In case you want to remove multiple tags from the user, you can either comma-separate them like <code>2,3,12,44</code>, or you can add them via a JSON construct:", 'wp-webhooks' ); ?>
<pre>{
  23,
  3,
  44
		...
		$parameter['tags']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo __( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>wpfs_remove_tags</strong> action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	...
	<li>
		...
		return array(
			...
			'name'			   => __( 'Remove tags', 'wp-webhooks' ),
			'sentence'			   => __( 'remove one or multiple tags', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => __( 'Remove one or multiple tags from a user within WP Fusion.', 'wp-webhooks' ),
			'description'	   => array(),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $user ) ){
				$return_args['msg'] = __( "Please set the user argument to either the user id or user email.", 'action-wpfs_remove_tags-error' );
				return $return_args;
			}

			if( empty( $tags ) ){
				$return_args['msg'] = __( "Please set the tags argument.", 'action-wpfs_remove_tags-error' );
				return $return_args;
			}
            ...
            if( empty( $user_id ) ){
                $return_args['msg'] = __( "We could not find a user for your given user id.", 'action-wpfs_remove_tags-error' );
				return $return_args;
            }
			...
			$return_args['msg'] = __( "Tags have been removed from the given user.", 'action-wpfs_remove_tags-success' );
