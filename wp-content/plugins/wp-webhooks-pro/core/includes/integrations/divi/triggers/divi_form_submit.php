  ...
  class WP_Webhooks_Integrations_divi_Triggers_divi_form_submit {
	...
	public function get_details(){
		...
		$parameter = array(
			...
			'submission' => array( 'short_description' => __( '(String) The data submitted with the form.', 'wp-webhooks' ) ),
			'errors' => array( 'short_description' => __( '(Array) Any kind of errors that appeared during the form submission.', 'wp-webhooks' ) ),
			'form_info' => array( 'short_description' => __( '(Array) All of the form fields that have been submitted.', 'wp-webhooks' ) ),
		);

		$description = array(
			'tipps' => array(
				__( 'You can fire this trigger as well on specific forms only. To do that, simply specify the form ids within the webhook URL settings.', 'wp-webhooks' ),
			)
		);
		...
		$settings = array(
			...
			'data' => array(
				'wpwhpro_divi_trigger_on_selected_forms' => array(
					...
					'placeholder' => '',
					'required'	=> false,
					'description' => __( 'Set the IDs of the forms you would like to fire this trigger on. Comma-separate them in case you want to add multiple ones. If none are set, all are triggered. To get the ID for your form, please head on the frontend page of your form, right-click it, and search for data-form_unique_id - this field contains an ID, which is your unique form id.', 'wp-webhooks' )
				),
			)
		);
		...
		return array(
			...
			'short_description' => __( 'This webhook fires as soon as a form was submitted within Divi.', 'wp-webhooks' ),
