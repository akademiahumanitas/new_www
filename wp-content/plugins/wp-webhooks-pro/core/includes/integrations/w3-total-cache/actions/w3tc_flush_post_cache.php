...
if ( ! class_exists( 'WP_Webhooks_Integrations_w3_total_cache_Actions_w3tc_flush_post_cache' ) ) :

	/**
	 * Load the w3tc_flush_post_cache action
	 *
	 * @since 6.0
	 * @author Ironikus <info@ironikus.com>
	 */
	class WP_Webhooks_Integrations_w3_total_cache_Actions_w3tc_flush_post_cache {
		...
		public function get_details() {
			...
			$returns_code = array(
				'success' => true,
				'msg'     => 'The full post cache has been flushed.',
			);

			return array(
				'action'            => 'w3tc_flush_post_cache',
				'name'              => __( 'Flush post cache', 'wp-webhooks' ),
				'sentence'          => __( 'flush the cache for one, multiple, or all posts', 'wp-webhooks' ),
				'parameter'         => $parameter,
				'returns'           => $returns,
				'returns_code'      => $returns_code,
				'short_description' => __( 'Flush the cache for one, multiple, or all posts within W3 Total Cache.', 'wp-webhooks' ),
				'description'       => array(),
		...
		public function execute( $return_data, $response_body ) {
			...
			if( ! empty( $post_ids ) ){
				...
				$return_args['msg']     = __( 'The given post IDs have been flushed.', 'wp-webhooks' );
			} else {
				w3tc_flush_posts();
				$return_args['msg']     = __( 'The full post cache has been flushed.', 'wp-webhooks' );
			}
