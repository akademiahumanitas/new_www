...
<pre>all</pre>
		...
		$parameter['event_ids']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo __( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>emng_approve_bookings</strong> action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	<li>
		...
		return array(
			...
			'name'			   => __( 'Approve bookings', 'wp-webhooks' ),
			'sentence'			   => __( 'approve one or multiple bookings', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => __( 'Approve one, multiple, or all bookings for a user within Events Manager.', 'wp-webhooks' ),
			'description'	   => array(),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $user ) ){
				$return_args['msg'] = __( "Please set the user argument to either the user id or user email.", 'action-emng_approve_bookings-error' );
				return $return_args;
			}

			if( empty( $event_ids ) ){
				$return_args['msg'] = __( "Please set the event_ids argument.", 'action-emng_approve_bookings-error' );
				return $return_args;
			}
            ...
            if( empty( $user_id ) ){
                $return_args['msg'] = __( "We could not find a user for your given user data.", 'action-emng_approve_bookings-error' );
				return $return_args;
            }
			...
			if( ! empty( $em_person ) ){
				...
				if( count( $em_bookings->bookings ) > 0 ){
					...
					$return_args['msg'] = __( "The bookings have been approved.", 'action-emng_approve_bookings-success' );
					...
					$return_args['msg'] = __( "The given user has no bookings.", 'action-emng_approve_bookings-success' );
				}
			} else {
				$return_args['msg'] = __( "We could not find a booking person for the given user.", 'action-emng_approve_bookings-success' );
			}
