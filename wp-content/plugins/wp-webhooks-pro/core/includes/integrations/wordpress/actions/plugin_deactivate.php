...
if ( ! class_exists( 'WP_Webhooks_Integrations_wordpress_Actions_plugin_deactivate' ) ) :
	...
	class WP_Webhooks_Integrations_wordpress_Actions_plugin_deactivate {
		...
		public function get_details(){
				...
				$parameter = array(
				'plugin_slug'	   => array( 'required' => true, 'short_description' => __( '(string) The plugin slug of the plugin you want to deactivate. Please check the description for further details.', 'wp-webhooks' ) ),
...
<?php
			...
			ob_start();
?>
<?php echo __( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>plugin_deactivate</strong> action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	...
	<li>
		...
		<?php echo __( "True if the plugin was successfully deactivated, false if not.", 'wp-webhooks' ); ?>
	</li>
	<li>
...
<?php
			...
			return array(
				...
				'name'			  => __( 'Deactivate plugin', 'wp-webhooks' ),
				'sentence'			  => __( 'deactivate a plugin', 'wp-webhooks' ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => __( 'This webhook action allows you to deactivate a plugin within your WordPress website.', 'wp-webhooks' ),
				'description'	   => array(),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $plugin_slug ) ){
				$return_args['msg'] = __( "Please set the plugin_slug to continue.", 'action-plugin_deactivate-failure' );
				return $return_args;
			}

			$check = $plugin_helpers->deactivate( $plugin_slug );
			if( $check ){
				$return_args['msg'] = __( "Plugin successfully deactivated.", 'action-plugin_deactivate-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = __( "The plugin was not deactivated.", 'action-plugin_deactivate-success' );
			}
