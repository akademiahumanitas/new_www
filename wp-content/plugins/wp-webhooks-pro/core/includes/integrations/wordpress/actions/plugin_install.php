...
if ( ! class_exists( 'WP_Webhooks_Integrations_wordpress_Actions_plugin_install' ) ) :
	...
	class WP_Webhooks_Integrations_wordpress_Actions_plugin_install {
		...
		public function get_details(){
				...
				$parameter = array(
				'plugin_slug'	   => array( 'required' => true, 'short_description' => __( '(string) The plugin slug of the plugin you want to install. Please check the description for further details.', 'wp-webhooks' ) ),
				'plugin_download'	 => array( 'required' => true, 'short_description' => __( '(String) The download URL of the plugin version you want to install. Please check the description for further details.', 'wp-webhooks' ) ),
...
<?php
			...
			ob_start();
?>
<?php echo __( "This argument contains the download URL to the ZIP file of your plugin. In case your plugin is hosted on WordPress.org, you can use the URL down below.", 'wp-webhooks' ); ?>
<pre>https://downloads.wordpress.org/plugin/wp-webhooks-comments.latest-stable.zip</pre>
<?php echo __( "The URL above adds the latest version of the plugin wp-webhooks-comments. If you want to customize it, just change the slug from <strong>wp-webhooks-comments</strong> to your WordPress slug.", 'wp-webhooks' ); ?>
<?php
			$parameter['plugin_download']['description'] = ob_get_clean();

			ob_start();
		?>
<?php echo __( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>plugin_install</strong> action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	...
	<li>
		...
		<?php echo __( "True if the plugin was successfully installed, false if not.", 'wp-webhooks' ); ?>
	</li>
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			return array(
				...
				'name'			  => __( 'Install plugin', 'wp-webhooks' ),
				'sentence'			  => __( 'install a plugin', 'wp-webhooks' ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => __( 'This webhook action allows you to install a plugin within your WordPress website.', 'wp-webhooks' ),
				'description'	   => array(),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $plugin_slug ) ){
				$return_args['msg'] = __( "Please set the plugin_slug to continue.", 'action-plugin_install-failure' );
				return $return_args;
			}
			
			if( empty( $plugin_download ) ){
				$return_args['msg'] = __( "Please set the plugin_download to continue.", 'action-plugin_install-failure' );
				return $return_args;
			}

			$check = $plugin_helpers->install( $plugin_slug, $plugin_download, array( 'prevent_outputs' => true ) );
			if( $check ){
				$return_args['msg'] = __( "Plugin successfully installed.", 'action-plugin_install-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = __( "The plugin was not installed.", 'action-plugin_install-success' );
			}
