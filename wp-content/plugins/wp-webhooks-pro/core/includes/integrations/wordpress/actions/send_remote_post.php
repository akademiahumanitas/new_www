...
if ( ! class_exists( 'WP_Webhooks_Integrations_wordpress_Actions_send_remote_post' ) ) :
	...
	class WP_Webhooks_Integrations_wordpress_Actions_send_remote_post {
		...
		public function get_details(){
			...
			$parameter = array(
				...
				'reject_unsafe_urls'	=> array( 'short_description' => __( '(bool) Filters whether to pass URLs through wp_http_validate_url() in an HTTP request. Default: no', 'wp-webhooks' ) ),
				'sslverify'	=> array( 
					'type' => 'select', 
					'choices' => array( 
						'yes' => __( 'Yes', 'wp-webhooks' ),
  ...
  "test-cookie": "The Test Cookie"
			...
			ob_start();
		?>
<?php echo __( "This argument allows you to change the mehtod of this request. Default is POST. Down below you will find a list of all available request methods:", 'wp-webhooks' ); ?>
<ul>
	<li>POST</li>
	<li>GET</li>
	<li>HEAD</li>
...
<?php echo __( "Set this argument to no to use unverified SSL connections for this URL.", 'wp-webhooks' ); ?>
		...
		$parameter['sslverify']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo __( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>send_remote_post</strong> action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	...
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			$description = array(
				'tipps' => array(
					__( "In case the requests fails, it's highly suggested to take a closer look at the <strong>sslverify</strong> and <strong>reject_unsafe_urls</strong> arguments.", 'wp-webhooks' ),
				),
			);

			return array(
				'action'			=> 'send_remote_post',
				'name'			  => __( 'Send remote HTTP POST', 'wp-webhooks' ),
				'sentence'			  => __( 'send a remote HTTP request', 'wp-webhooks' ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => __( 'This webhook action allows you to send a HTTP request from your WordPress site.', 'wp-webhooks' ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $url ) ){
				$return_args['msg'] = __( "Please set the url argument.", 'action-send_remote_post-failure' );
				return $return_args;
			}
			...
			if( $check && ! is_wp_error( $check ) ){
				$return_args['msg'] = __( "The request was sent successfully.", 'action-send_remote_post-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = __( "An error occured while sending the request.", 'action-send_remote_post-success' );
			}
