...
if ( ! class_exists( 'WP_Webhooks_Integrations_wordpress_Actions_plugin_update' ) ) :
	...
	class WP_Webhooks_Integrations_wordpress_Actions_plugin_update {
		...
		public function get_details(){
				...
				$parameter = array(
				'plugin_slug'	   => array( 'required' => true, 'short_description' => __( '(string) The plugin slug of the plugin you want to update. Please check the description for further details.', 'wp-webhooks' ) ),
...
<?php
			...
			ob_start();
?>
<?php echo __( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>plugin_update</strong> action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	...
	<li>
		...
		<?php echo __( "True if the plugin was successfully updated, false if not.", 'wp-webhooks' ); ?>
	</li>
	<li>
...
<?php
			...
			$description = array(
				'tipps' => array(
					__( '<strong>Please note</strong>: The update of a plugin requires the plugin to be hosted on WordPress.org or by using a custom update class like Easy Digital Downloads offers it.', 'wp-webhooks' ),
				),
			);

			return array(
				'action'			=> 'plugin_update',
				'name'			  => __( 'Update plugin', 'wp-webhooks' ),
				'sentence'			  => __( 'update a plugin', 'wp-webhooks' ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => __( 'This webhook action allows you to update a plugin within your WordPress website.', 'wp-webhooks' ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $plugin_slug ) ){
				$return_args['msg'] = __( "Please set the plugin_slug to continue.", 'action-plugin_update-failure' );
				return $return_args;
			}

			$check =$plugin_helpers->update( $plugin_slug, array( 'prevent_outputs'  =>  true ) );
			if( $check ){
				$return_args['msg'] = __( "Plugin successfully updated.", 'action-plugin_update-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = __( "The plugin was not updated.", 'action-plugin_update-success' );
			}
