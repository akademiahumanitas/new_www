...
if ( ! class_exists( 'WP_Webhooks_Integrations_wordpress_Actions_wp_manage_user_meta_data' ) ) :
	...
	public function get_details(){
		...
		$parameter = array(
			'user_id' => array( 'required' => true, 'short_description' => __( 'The ID (or the email) of the user you want to perform the action for.', 'wp-webhooks' ) ),
			'meta_update' => array( 
				'type' => 'repeater',
...
<ol>
   ...
   "delete_user_meta":[
	  ...
	  },
	  {
		...
		<?php echo __( "Within the example above, you will see that only the meta key is required for deleting an entry. This will cause all meta keys on this user with the same key to be deleted.", 'wp-webhooks' ); ?><br>
		<?php echo __( "The second argument allows you to target only a specific meta key/value combination. This gets important if you want to target a specific meta key/value combination and not delete all available entries for the given user. Default: ''", 'wp-webhooks' ); ?>
	</li>
</ol>
...
<ol>
	...
	<li><?php echo __( "The webhook response contains a validted array that shows each initialized meta entry, as well as the response from its original WordPress function. This way you can see if the meta value was adjusted accordingly.", 'wp-webhooks' ); ?></li>
		...
		$parameter['manage_meta_data']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo __( "The do_action argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the wp_manage_user_meta_data action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	...
	<li>
		...
		<?php echo __( "An array containing further data about the simplified meta data that should be added/updated.", 'wp-webhooks' ); ?>
	</li>
</ol>
		<?php
		$parameter['do_action']['description'] = ob_get_clean();
			...
			$description = array(
				'tipps' => array(
					__( "To create user meta values visually, you can use our meta value generator at: <a title=\"Visit our meta value generator\" href=\"https://wp-webhooks.com/blog/how-to-update-custom-post-meta-values-with-wp-webhooks/\" target=\"_blank\">https://wp-webhooks.com/blog/how-to-update-custom-post-meta-values-with-wp-webhooks/</a>.", 'wp-webhooks' ),
				)
			);

			return array(
				'action'			=> 'wp_manage_user_meta_data',
				'name'			  => __( 'Manage WP user meta', 'wp-webhooks' ),
				'sentence'			  => __( 'add, update, or delete WP user meta', 'wp-webhooks' ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => __( 'Add, update, or delete custom user meta data within "WordPress".', 'wp-webhooks' ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $user_id ) ){
				$return_args['msg'] = __( "Please set the user_id argument first.", 'wp-webhooks' );
				return $return_args;
			}
			...
			if( empty( $user_id ) || ! is_numeric( $user_id ) ){
				$return_args['msg'] = __( "The user you try to update does not exist.", 'wp-webhooks' );
				return $return_args;
			}
