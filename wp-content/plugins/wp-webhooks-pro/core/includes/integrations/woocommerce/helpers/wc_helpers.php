...
if ( ! class_exists( 'WP_Webhooks_Integrations_woocommerce_Helpers_wc_helpers' ) ) :
	...
	class WP_Webhooks_Integrations_woocommerce_Helpers_wc_helpers {
		...
		private $first_admin_cache = null;

		/**
		 * Get the first admin if a user
		 *
		 * @since 6.1.4
		 * @return integer The first admin ID
		 */
		public function get_first_admin_id(){
			$admin_id = 0;

			if( ! empty( $this->first_admin_cache ) ){
				$admin_id = $this->first_admin_cache;
			} else {
				...
				if( ! empty( $user_query ) ){
					...
					if( is_array( $user_results ) ){
						foreach( $user_results as $admin ){
							$admin_id = $admin->ID;
							$this->first_admin_cache = $admin_id;
							break;
						}
					}
			...
			return apply_filters( 'wpwh/integrations/wc/helpers/get_first_admin', $admin_id );
		}
		...
		public function get_wc_api_versions(){
			...
			foreach( $versions as $version ){
				$validated_versions[ $version ] = esc_html( sprintf( __( 'WP REST API v%d', 'wp-webhooks' ), str_replace( 'wp_api_v', '', $version ) ) );
			}
