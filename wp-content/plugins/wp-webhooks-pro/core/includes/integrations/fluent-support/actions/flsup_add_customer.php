<?php
if ( ! class_exists( 'WP_Webhooks_Integrations_fluent_support_Actions_flsup_add_customer' ) ) :
	/**
	 * Load the flsup_add_customer action
	 *
	 * @since 6.0.1
	 * @author Ironikus <info@ironikus.com>
	 */
	class WP_Webhooks_Integrations_fluent_support_Actions_flsup_add_customer {
		...
		public function get_details() {
			$parameter = array(
				...
				'status'         => array(
					...
					'short_description' => __( '(String) The customer status.', 'wp-webhooks' ),
				),
				'note'           => array(
			...
			return array(
				'action'            => 'flsup_add_customer', // required
				'name'              => __( 'Add customer', 'wp-webhooks' ),
				'sentence'          => __( 'add a customer', 'wp-webhooks' ),
				'parameter'         => $parameter,
				'returns'           => $returns,
				'returns_code'      => $returns_code,
				'short_description' => __( 'Add a customer within Fluent Support.', 'wp-webhooks' ),
				'description'       => array(),
		...
		public function execute( $return_data, $response_body ) {
			...
			if ( isset( $exists->id ) && $exists->id > 0 && $exists instanceof \FluentSupport\App\Models\Customer ) {
				$return_args['msg'] = __( 'The customer with the given email address already exists. Please set the unique email argument.', 'wp-webhooks' );
				return $return_args;
			}
			...
			if ( ! empty( $new_customer ) && $new_customer instanceof \FluentSupport\App\Models\Customer ) {
				...
				$return_args['msg']     = __( 'The customer has been added successfully.', 'wp-webhooks' );
				$return_args['data']    = $new_customer;
			} else {
				$return_args['msg'] = __( 'An error occured while adding the customer.', 'wp-webhooks' );
			}
