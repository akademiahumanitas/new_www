  ...
  */
  class WP_Webhooks_Integrations_webhooks_Triggers_received_http_request {
	...
	public function get_details(){
		...
		$parameter = array(
			'custom_construct' => array( 'short_description' => __( '(Mixed) The data that was sent along with the HTTP call that was made to the receivable URL.', 'wp-webhooks' ) ),
		);

		$description = array(
			'steps' => array(
				__( 'Within the settings of this trigger, copy the receivable URL (The dynamically created URL).', 'wp-webhooks' ),
				__( 'Place the receivable URL into your service of choice to send data to this trigger.', 'wp-webhooks' ),
			),
			'tipps' => array(
		...
		return array(
			...
			'name'			  => __( 'HTTP request received', 'wp-webhooks' ),
			'sentence'			  => __( 'a HTTP request was received', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => sprintf( __( 'This webhook fires an outbound API call as soon as a request was received on the receivable URL within the "Webhooks" integration.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() ),
