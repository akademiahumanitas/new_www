...
if ( ! class_exists( 'WP_Webhooks_Integrations_fluent_crm_Actions_fcrm_contact_to_tag' ) ) :
	...
	public function get_details(){
			...
			$parameter = array(
				'email'			=> array( 'short_description' => __( 'Set the email of the contact/user you want to add the tags to.', 'wp-webhooks' ) ),
				...
				'tags'	=> array( 'required' => true, 'short_description' => __( 'Add the tags you want to add the user to. This argument accepts a comma-separated string, as well as a JSON construct.', 'wp-webhooks' ) ),
			...
			ob_start();
		?>
<?php echo __( "In case you want to add multiple tags to the contact, you can either comma-separate them like <code>2,3,12,44</code>, or you can add them via a JSON construct:", 'wp-webhooks' ); ?>
<pre>{
  23,
  3,
  44
		...
		$parameter['tags']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo __( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>fcrm_contact_to_tag</strong> action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	...
	<li>
		...
		<?php echo __( "The email of the contact we add the tags to.", 'wp-webhooks' ); ?>
	</li>
	<li>
	...
	<li>
		...
		<?php echo __( "An array of the tags that have been added to the user.", 'wp-webhooks' ); ?>
	</li>
</ol>
		<?php
		...
		return array(
			...
			'name'			   => __( 'Add contact to tag', 'wp-webhooks' ),
			'sentence'			   => __( 'add a contact to a tag', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => __( 'Add a contact to one or multiple tags within FluentCRM.', 'wp-webhooks' ),
			'description'	   => array(),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $email ) && empty( $user_id ) ){
				$return_args['msg'] = __( "Please set either the user_email or the user_id argument.", 'action-fcrm_contact_to_tag-error' );
				return $return_args;
			}

			if( empty( $tags ) ){
				$return_args['msg'] = __( "Please set the tags argument.", 'action-fcrm_contact_to_tag-error' );
				return $return_args;
			}
            ...
            if( empty( $validated_user_email ) ){
                $return_args['msg'] = __( "We could not find a contact for your given email or id.", 'action-fcrm_contact_to_tag-error' );
				return $return_args;
            }

            $contact = $fcrm_helpers->get_contact( 'email', $validated_user_email );
			if( empty( $contact ) ) {
                $return_args['msg'] = __( "We could not fetch the tags from your given contact.", 'action-fcrm_contact_to_tag-error' );
				return $return_args;
            }
			...
			if( is_array( $validated_tags ) ){
				...
				$return_args['msg'] = __( "Tags have been added to the given contact.", 'action-fcrm_contact_to_tag-success' );
				$return_args['data']['contact'] = $contact;
			} else {
				$return_args['msg'] = __( "Error: There was an issue validating the tags.", 'action-fcrm_contact_to_tag-error' );
			}
