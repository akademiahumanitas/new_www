...
if ( ! class_exists( 'WP_Webhooks_Integrations_restrict_content_pro_Actions_rcp_expire_membership' ) ) :
	...
	public function get_details(){
			...
			$parameter = array(
				...
				'membership_level'	=> array( 'required' => true, 'short_description' => __( 'The ID of the membership level that you want to expire. Set this argument to all to expire all memberships.', 'wp-webhooks' ) ),
			...
			ob_start();
		?>
<?php echo __( "You can set this argument to <code>all</code> to expire all memberships for the user instead.", 'wp-webhooks' ); ?>
		<?php
		$parameter['membership_level']['description'] = ob_get_clean();

			ob_start();
		?>
<?php echo __( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>rcp_expire_membership</strong> action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	...
	<li>
		...
		<?php echo __( "The data used to expire the membership.", 'wp-webhooks' ); ?>
	</li>
</ol>
		<?php
		...
		$description = array(
			'tipps' => array(
				__( 'Please note that expiring a membership will set its expiry date to the day before you call the webhook.', 'wp-webhooks' )
			),
		);

		return array(
			'action'			=> 'rcp_expire_membership', //required
			'name'			   => __( 'Expire user membership', 'wp-webhooks' ),
			'sentence'			   => __( 'expire one or all user memberships', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => __( 'Expire one or all memberships for a user within Restrict Content Pro.', 'wp-webhooks' ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $user ) ){
				$return_args['msg'] = __( "Please set the user argument to either the user id or user email of an existing user.", 'action-rcp_expire_membership-error' );
				return $return_args;
			}

			if( empty( $membership_level ) ){
				$return_args['msg'] = __( "Please set the membership_level argument.", 'action-rcp_expire_membership-error' );
				return $return_args;
			}
            ...
            if( empty( $user_id ) ){
                $return_args['msg'] = __( "We could not find a user for your given user argument value.", 'action-rcp_expire_membership-error' );
				return $return_args;
            }
			...
			if( empty( $customer ) ){
                $return_args['msg'] = __( "There was an issue retrieving the customer.", 'action-rcp_expire_membership-error' );
				return $return_args;
            }
			...
			$return_args['msg'] = __( "The memberships have been successfully expired.", 'action-rcp_expire_membership-success' );
