...
if ( ! class_exists( 'WP_Webhooks_Integrations_restrict_content_pro_Actions_rcp_cancel_membership' ) ) :
	...
	public function get_details(){
			...
			$parameter = array(
				...
				'membership_level'	=> array( 'required' => true, 'short_description' => __( 'The ID of the membership level that you want to cancel. Set this argument to all to cancel all memberships.', 'wp-webhooks' ) ),
			...
			ob_start();
		?>
<?php echo __( "You can set this argument to <code>all</code> to cancel all memberships for the user instead.", 'wp-webhooks' ); ?>
		<?php
		$parameter['membership_level']['description'] = ob_get_clean();

			ob_start();
		?>
<?php echo __( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>rcp_cancel_membership</strong> action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	...
	<li>
		...
		<?php echo __( "The data used to cancel the membership.", 'wp-webhooks' ); ?>
	</li>
</ol>
		<?php
		...
		return array(
			...
			'name'			   => __( 'Cancel user membership', 'wp-webhooks' ),
			'sentence'			   => __( 'cancel one or all user memberships', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => __( 'Cancel one or all memberships for a user within Restrict Content Pro.', 'wp-webhooks' ),
			'description'	   => array(),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $user ) ){
				$return_args['msg'] = __( "Please set the user argument to either the user id or user email of an existing user.", 'action-rcp_cancel_membership-error' );
				return $return_args;
			}

			if( empty( $membership_level ) ){
				$return_args['msg'] = __( "Please set the membership_level argument.", 'action-rcp_cancel_membership-error' );
				return $return_args;
			}
            ...
            if( empty( $user_id ) ){
                $return_args['msg'] = __( "We could not find a user for your given user argument value.", 'action-rcp_cancel_membership-error' );
				return $return_args;
            }
			...
			if( empty( $customer ) ){
                $return_args['msg'] = __( "There was an issue retrieving the customer.", 'action-rcp_cancel_membership-error' );
				return $return_args;
            }
			...
			$return_args['msg'] = __( "The memberships have been successfully cancelled.", 'action-rcp_cancel_membership-success' );
