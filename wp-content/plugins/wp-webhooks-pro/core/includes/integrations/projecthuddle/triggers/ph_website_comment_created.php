...
if ( ! class_exists( 'WP_Webhooks_Integrations_projecthuddle_Triggers_ph_website_comment_created' ) ) :

 /**
  * Load the ph_website_comment_created trigger
  *
  * @since 6.0
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_projecthuddle_Triggers_ph_website_comment_created {
	...
	public function get_callbacks(){
		...
		return array(
			array(
				'type' => 'action',
				'hook' => 'ph_website_publish_comment',
				'callback' => array( $this, 'ph_website_comment_created_callback' ),
				'priority' => 20,
				'arguments' => 2,
				'delayed' => true,
			),
	...
	public function get_details(){
		...
		$parameter = array(
			...
			'website_id' => array( 'short_description' => __( '(Integer) The id of the connected website.', 'wp-webhooks' ) ),
			...
			'website_url' => array( 'short_description' => __( '(String) The URL of the connected website.', 'wp-webhooks' ) ),
			'website_data' => array( 'short_description' => __( '(Array) The details about the connected website.', 'wp-webhooks' ) ),
			'website_meta' => array( 'short_description' => __( '(Array) Further details about the connected website.', 'wp-webhooks' ) ),
		);
		...
		$settings = array(
			...
			'data' => array(
				'wpwhpro_projecthuddle_trigger_on_approved_comments' => array(
					...
					'label'	   => __( 'Trigger on comment approval status', 'wp-webhooks' ),
					'placeholder' => '',
					'required'	=> false,
				...
				'wpwhpro_projecthuddle_trigger_on_website' => array(
					'id'			=> 'wpwhpro_projecthuddle_trigger_on_website',
					'type'			=> 'select',
					'multiple'		=> true,
 					'choices'		=> array(),
 					'query'			=> array(
					...
					'label'			=> __( 'Trigger on websites', 'wp-webhooks' ),
					'placeholder'	=> '',
					'required'		=> false,
					'description'	=> __( 'Fire this trigger on specific websites only. If none are selected, all are triggered.', 'wp-webhooks' )
				),
			)
		);

		return array(
			'trigger'		   => 'ph_website_comment_created',
			'name'			  => __( 'Website comment created', 'wp-webhooks' ),
			'sentence'			  => __( 'a website comment was created', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => __( 'This webhook fires as soon as a website comment was created within ProjectHuddle.', 'wp-webhooks' ),
			'description'	   => array(),
	...
	public function ph_website_comment_created_callback( $id, $comment ){

		$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'ph_website_comment_created' );
		...
		$payload = array(
			...
			'website_url' => get_permalink( $page_id ),
			'website_data' => get_post( $page_id ),
			'website_meta' => get_post_meta( $page_id ),
		);
		...
		if( is_array( $payload['website_meta'] ) && ! empty( $payload['website_meta'] ) && isset( $payload['website_meta']['html'] ) ){
			unset( $payload['website_meta']['html'] );
		}
		...
		foreach( $webhooks as $webhook ){
			...
			if( isset( $webhook['settings'] ) ){
				foreach( $webhook['settings'] as $settings_name => $settings_data ){
				  ...
				  if( $is_valid && $settings_name === 'wpwhpro_projecthuddle_trigger_on_website' && ! empty( $settings_data ) ){
					$is_valid = false;
		...
		do_action( 'wpwhpro/webhooks/trigger_ph_website_comment_created', $payload, $response_data_array );
	}
	...
	public function get_demo( $options = array() ) {
		...
		$data = array (
			...
			'website_url' => 'https://yourdomain.test/the-page-slug/',
			'website_data' =>
			array (
				'ID' => 9371,
				'post_author' => '1',
