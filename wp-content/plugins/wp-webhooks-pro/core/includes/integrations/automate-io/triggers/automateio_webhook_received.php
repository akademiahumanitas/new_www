  ...
  */
  class WP_Webhooks_Integrations_automate_io_Triggers_automateio_webhook_received {
	...
	public function get_details(){
		...
		$parameter = array(
			'custom_construct' => array( 'short_description' => __( '(Mixed) The data that was sent along with the webhooks call that was made to the receivable URL from within Automate.io.', 'wp-webhooks' ) ),
		);

		$description = array(
			'steps' => array(
				__( 'Within the settings of this webhook trigger, copy the receivable URL.', 'wp-webhooks' ),
				__( 'Head into Automate.io and select the "Webhooks" app along with the "POST data" action.', 'wp-webhooks' ),
				__( 'Place the receivable URL there and send data based on your requirements.', 'wp-webhooks' ),
			),
			'tipps' => array(
				__( 'To receive data on the receivable URL, please use the "Webhooks" app within Automate.io along with the "POST data" action.', 'wp-webhooks' ),
		...
		return array(
			...
			'name'			  => __( 'Automate.io webhook request received', 'wp-webhooks' ),
			'sentence'			  => __( 'a Automate.io webhook request was received', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => sprintf( __( 'This webhook fires as soon as a request was received from the "Webhooks" app of Automate.io via the "POST data" action.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() ),
