<?php
if ( ! class_exists( 'WP_Webhooks_Integrations_sql_Actions_sql_run_remote_queries' ) ) :
	/**
	 ...
	 */
	class WP_Webhooks_Integrations_sql_Actions_sql_run_remote_queries {
		...
		public function get_details() {
			...
			$parameter         = array(
				'db_host' => array(
					'required'          => true,
					'label'             => __( 'Database host', 'wp-webhooks' ),
					'short_description' => __( '(String) The database host. E.g. localhost', 'wp-webhooks' ),
				),
				'db_user' => array(
					'required'          => true,
					'label'             => __( 'Database user', 'wp-webhooks' ),
					'short_description' => __( '(String) The username of the database. E.g. root', 'wp-webhooks' ),
				),
				'db_password' => array(
					'required'          => false,
					'label'             => __( 'Database password', 'wp-webhooks' ),
					'short_description' => __( '(String) The password for the user of the database.', 'wp-webhooks' ),
				),
				'db_name' => array(
					'required'          => true,
					'label'             => __( 'Database name', 'wp-webhooks' ),
					'short_description' => __( '(String) The name of the database.', 'wp-webhooks' ),
				),
				'queries' => array(
					'required'          => true,
...
<?php
			...
			return array(
				...
				'name'              => __( 'Run remote SQL queries', 'wp-webhooks' ),
				'sentence'          => __( 'run one or multiple remote SQL queries', 'wp-webhooks' ),
				'parameter'         => $parameter,
				'returns'           => $returns,
				'returns_code'      => $returns_code,
				'short_description' => __( 'Run one or multiple, arbitrary SQL queries against a given local or remote database.', 'wp-webhooks' ),
				'description'       => array(),
				'integration'       => 'sql',
				'premium'           => true,
			);
		...
		public function execute( $return_data, $response_body ) {
			...
			$db_host = WPWHPRO()->helpers->validate_request_value( $response_body['content'], 'db_host' );
			$db_user = WPWHPRO()->helpers->validate_request_value( $response_body['content'], 'db_user' );
			$db_password = WPWHPRO()->helpers->validate_request_value( $response_body['content'], 'db_password' );
			$db_name = WPWHPRO()->helpers->validate_request_value( $response_body['content'], 'db_name' );
			...
			if( empty( $dbconnect ) ){
				$return_args['msg']     = __( 'we could not connect to the given database.', 'wp-webhooks' );
				return $return_args;
			}
