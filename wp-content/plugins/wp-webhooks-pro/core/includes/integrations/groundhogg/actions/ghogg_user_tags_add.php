...
if ( ! class_exists( 'WP_Webhooks_Integrations_groundhogg_Actions_ghogg_user_tags_add' ) ) :
	...
	public function get_details(){
			...
			$parameter = array(
				...
				'tags'	=> array( 'required' => true, 'short_description' => __( 'Add the tags you want to add to the contact. This argument accepts a comma-separated string, as well as a JSON construct.', 'wp-webhooks' ) ),
...
<pre>{
  ...
  44
		...
		$parameter['tags']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo __( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>ghogg_user_tags_add</strong> action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	...
	<li>
		...
		return array(
			...
			'name'			   => __( 'Add user tags', 'wp-webhooks' ),
			'sentence'			   => __( 'add one or multiple tags to a user', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => __( 'Add one or multiple tags to a user within Groundhogg.', 'wp-webhooks' ),
			'description'	   => array(),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $contact_value ) ){
				$return_args['msg'] = __( "Please set the contact_value argument.", 'action-ghogg_user_tags_add-error' );
				return $return_args;
			}

			if( empty( $tags ) ){
				$return_args['msg'] = __( "Please set the tags argument.", 'action-ghogg_user_tags_add-error' );
				return $return_args;
			}
			...
			if( ! $contact->exists() ) {
				$return_args['msg'] = __( "The contact you try to update does not exist.", 'action-ghogg_user_tags_add-error' );
				return $return_args;
			}
			...
			$return_args['msg'] = __( "Tags have been added to the respective contact.", 'action-ghogg_user_tags_add-success' );
