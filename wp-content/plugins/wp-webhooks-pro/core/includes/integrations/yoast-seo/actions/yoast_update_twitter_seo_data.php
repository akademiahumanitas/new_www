<?php
if ( ! class_exists( 'WP_Webhooks_Integrations_yoast_seo_Actions_yoast_update_twitter_seo_data' ) ) :
	/**
	 * Load the yoast_update_twitter_seo_data action
	 *
	 * @since 6.1.1
	 * @author Ironikus <info@ironikus.com>
	 */
	class WP_Webhooks_Integrations_yoast_seo_Actions_yoast_update_twitter_seo_data {
		...
		public function get_details() {
			$parameter = array(
				...
				'title'       => array(
					...
					'short_description' => __( '(String) The Twitter SEO title.', 'wp_webhooks' ),
				),
				'description' => array(
					'label'             => __( 'Description', 'wp_webhooks' ),
					'short_description' => __( '(String) The Twitter SEO description.', 'wp_webhooks' ),
				),
				'image'       => array(
					'label'             => __( 'Twitter image', 'wp_webhooks' ),
					'type'              => 'select',
					'query'             => array(
						'filter' => 'posts',
						'args'   => array(
			...
			$returns_code = array(
				'success' => true,
				'msg'     => 'The Twitter SEO data has been updated successfully.',
			);

			return array(
				'action'            => 'yoast_update_twitter_seo_data', // required
				'name'              => __( 'Update Twitter SEO data', 'wp-webhooks' ),
				'sentence'          => __( 'update the Twitter SEO data', 'wp-webhooks' ),
				'parameter'         => $parameter,
				'returns'           => $returns,
				'returns_code'      => $returns_code,
				'short_description' => __( 'Update the Twitter SEO data within Yoast SEO.', 'wp-webhooks' ),
				'description'       => array(),
		...
		public function execute( $return_data, $response_body ) {
			...
			if ( ! empty( $metatitle ) ) {
				update_post_meta( $post_id, '_yoast_wpseo_twitter-title', $metatitle );
			}

			if ( ! empty( $metadesc ) ) {
				update_post_meta( $post_id, '_yoast_wpseo_twitter-description', $metadesc );
			}

			if ( ! empty( $image_id ) && get_post_type( $image_id ) === 'attachment' ) {
				update_post_meta( $post_id, '_yoast_wpseo_twitter-image-id', $image_id );
			...
			$return_args['msg']                 = __( 'The Twitter SEO data has been updated successfully.', 'wp-webhooks' );
