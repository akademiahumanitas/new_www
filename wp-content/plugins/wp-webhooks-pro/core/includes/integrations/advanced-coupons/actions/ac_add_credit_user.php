...
if ( ! class_exists( 'WP_Webhooks_Integrations_advanced_coupons_Actions_ac_add_credit_user' ) ) :
	/**
	 * Load the ac_add_credit_user action
	 *
	 * @since 6.1.1
	 * @author Ironikus <info@ironikus.com>
	 */
	class WP_Webhooks_Integrations_advanced_coupons_Actions_ac_add_credit_user {

		public function __construct(){
			add_filter(
				'acfw_get_store_credits_increase_source_types',
				array( $this, 'get_custom_source_type' )
			);	
		}
		...
		public function get_details() {
			$parameter = array(
				...
				'amount' => array(
					...
					'short_description' => __( '(Floatval) The amount of the credit you want to add.', 'wp_webhooks' ),
				),
			);
			...
			return array(
				'action'            => 'ac_add_credit_user', // required
				'name'              => __( 'Add user credit', 'wp-webhooks' ),
				'sentence'          => __( 'add credit to a user', 'wp-webhooks' ),
				'parameter'         => $parameter,
				'returns'           => $returns,
				'returns_code'      => $returns_code,
				'short_description' => __( 'Add credit to a user within Advanced Coupons.', 'wp-webhooks' ),
				'description'       => array(),
		...
		public function get_custom_source_type(){
			$custom_source_type = array(
				'admin_increase_wpwh' => array(
					'name'    => __( 'WP Webhooks (increase)', 'wp-webhooks' ),
					'slug'    => 'admin_increase_wpwh',
					'related' => array(
						'object_type' => 'wp-webhooks-link',
						'admin_label' => __( 'WP Webhooks Pro', 'wp-webhooks' ),
						'label'       => __( 'WP Webhooks Pro', 'wp-webhooks' ),
					),
				),
			);
