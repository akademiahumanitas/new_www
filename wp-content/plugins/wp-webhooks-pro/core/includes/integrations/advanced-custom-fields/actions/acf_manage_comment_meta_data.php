...
if ( ! class_exists( 'WP_Webhooks_Integrations_advanced_custom_fields_Actions_acf_manage_comment_meta_data' ) ) :
	...
	public function get_details(){
		...
		$parameter = array(
			'comment_id' => array( 'required' => true, 'short_description' => __( 'The ID of the comment you want to perform the action for.', 'wp-webhooks' ) ),
			'meta_update' => array( 
				'type' => 'repeater',
...
<pre>
{
	"meta_key": "Meta Value"
		...
		$parameter['manage_acf_data']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo __( "The do_action argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the acf_manage_comment_meta_data action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	...
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			$description = array(
				'tipps' => array(
					__( "To create comment meta values visually, you can use our meta value generator at: <a title=\"Visit our meta value generator\" href=\"https://wp-webhooks.com/blog/how-to-update-custom-post-meta-values-with-wp-webhooks/\" target=\"_blank\">https://wp-webhooks.com/blog/how-to-update-custom-post-meta-values-with-wp-webhooks/</a>.", 'wp-webhooks' ),
				)
			);

			return array(
				'action'			=> 'acf_manage_comment_meta_data',
				'name'			  => __( 'Manage ACF comment meta', 'wp-webhooks' ),
				'sentence'			  => __( 'add, update, or delete ACF comment meta', 'wp-webhooks' ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => __( 'Add, update, or delete custom comment meta data within "Advanced Custom Fields".', 'wp-webhooks' ),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $comment_id ) ){
				$return_args['msg'] = __( "Please set the comment_id argument first.", 'wp-webhooks' );
				return $return_args;
			}

			$comment = get_comment( $comment_id );
			if( empty( $comment ) ){
				$return_args['msg'] = __( "The comment you try to update does not exist.", 'wp-webhooks' );
				return $return_args;
			}
