<?php
if ( ! class_exists( 'WP_Webhooks_Integrations_bounceless_Actions_bounceless_verify_email' ) ) :

	/**
	 * Load the bounceless_verify_email action
	 *
	 * @since 6.1.3
	 * @author Ironikus <info@ironikus.com>
	 */
	class WP_Webhooks_Integrations_bounceless_Actions_bounceless_verify_email {
	...
	public function get_details(){
		...
		$parameter = array(
			'auth_template'		=> array( 
				...
				'query'			=> array(
					...
					'args'		=> array(
						'auth_methods' => array( 'bounceless_auth' )
					)
				),
				'short_description' => __( 'Use globally defined Bounceless credentials to authenticate this action.', 'wp-webhooks' ),
				'description' => __( 'This argument accepts the ID of a Bounceless authentication template of your choice. You can create new templates within the "Authentication" tab.', 'wp-webhooks' ),
			),
			'email'		=> array( 
				'required' => true, 
		...
		return array(
			'action'			=> 'bounceless_verify_email', //required
			'name'			   => __( 'Verify email', 'wp-webhooks' ),
			'sentence'			   => __( 'verify the validity of an email', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => __( 'Verify the validity and functionality of an email within "Bounceless".', 'wp-webhooks' ),
			'description'	   => array(),
			'integration'	   => 'bounceless',
			'premium'	   	=> true
		);
		...
		public function execute( $return_data, $response_body ){
			...
			if( ! empty( $auth_template ) ){
				$bounceless_auth = WPWHPRO()->integrations->get_auth( 'bounceless', 'bounceless_auth' );
				$credentials = $bounceless_auth->get_credentials( $auth_template );

				if( 
					isset( $credentials['wpwhpro_bounceless_api_key'] )
					&& ! empty( $credentials['wpwhpro_bounceless_api_key'] )
				){
					$api_key = $credentials['wpwhpro_bounceless_api_key'];
				}
			}
			...
			$api_url = 'https://apps.bounceless.io/api/verifyEmail?secret=' . $api_key . '&email=' . rawurlencode( $email );
