  ...
  */
  class WP_Webhooks_Integrations_klicktipp_Triggers_klickt_outbound_received {
	...
	public function get_details(){
		...
		$parameter = array(
			'custom_construct' => array( 'short_description' => __( '(Mixed) The data that was sent along with the HTTP call that was made to the receivable URL from within KlickTipp.', 'wp-webhooks' ) ),
		);

		$description = array(
			'steps' => array(
				__( 'Add a URL to this trigger on which you want to receive the KlickTipp data.', 'wp-webhooks' ),
				__( 'Go into the settings for your added URL and copy the receivable URL (The dynamically created URL).', 'wp-webhooks' ),
				__( 'Head into KlickTipp and select the "Webhooks by KlickTipp" app along with the "POST" or "Custom Request" action.', 'wp-webhooks' ),
				__( 'Place the receivable URL there and send data based on your requirements.', 'wp-webhooks' ),
			),
			'tipps' => array(
				__( 'To receive data on the receivable URL, please use the "Webhooks by KlickTipp" app within KlickTipp.', 'wp-webhooks' ),
		...
		return array(
			...
			'name'			  => __( 'KlickTipp outbound request received', 'wp-webhooks' ),
			'sentence'			  => __( 'a KlickTipp outbound request was received', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => sprintf( __( 'This webhook fires as soon as a request was received from the "Webhooks by KlickTipp" app of KlickTipp with the action "POST" or "Custom Request".', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() ),
