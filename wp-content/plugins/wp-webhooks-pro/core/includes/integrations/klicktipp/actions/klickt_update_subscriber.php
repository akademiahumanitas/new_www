	...
	public function get_details(){
		...
		$parameter = array(
			...
			'new_email_address'		=> array(
				'label' => __( 'New email address', 'wp-webhooks' ), 
				'short_description' => __( 'A new email address of the subscriber you want to update within KlickTipp.', 'wp-webhooks' ),
			),
			'fields' => array(
				'type' => 'repeater',
		...
		return array(
			...
			'name'			   => __( 'Update subscriber', 'wp-webhooks' ),
			'sentence'			   => __( 'update a subscriber', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => __( 'Update a subscriber within "KlickTipp".', 'wp-webhooks' ),
			'description'	   => array(),
		...
		public function execute( $return_data, $response_body ){
			...
			if( ! empty( $connector ) ){
				...
				if( ! empty( $subscriber ) ){
					...
					$return_args['msg'] = __( "The subscriber was successfully updated.", 'wp-webhooks' );
				} else {
					$return_args['msg'] = __( "An error occured while updating the subscriber.", 'wp-webhooks' );
