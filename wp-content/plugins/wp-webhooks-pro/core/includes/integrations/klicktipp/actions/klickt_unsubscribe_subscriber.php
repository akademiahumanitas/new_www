	...
	public function get_details(){
		...
		$parameter = array(
			...
			'email_address'		=> array( 
				...
				'short_description' => __( 'The email address of the subscriber you want to unsubscribe within KlickTipp.', 'wp-webhooks' ),
			),
		);
		...
		return array(
			...
			'name'			   => __( 'Unsubscribe subscriber', 'wp-webhooks' ),
			'sentence'			   => __( 'unsubscribe a subscriber', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => __( 'Unsubscribe a subscriber within "KlickTipp".', 'wp-webhooks' ),
			'description'	   => array(),
		...
		public function execute( $return_data, $response_body ){
			...
			if( ! empty( $connector ) ){
				...
				if( ! empty( $subscriber ) ){
					...
					$return_args['msg'] = __( "The subscriber was successfully unsubscribed.", 'wp-webhooks' );
				} else {
					$return_args['msg'] = __( "An error occured while unsubscribing the subscriber.", 'wp-webhooks' );
