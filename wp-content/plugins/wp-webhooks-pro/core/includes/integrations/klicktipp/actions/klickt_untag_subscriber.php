	...
	public function get_details(){
		...
		$parameter = array(
			...
			'email_address'		=> array( 
				...
				'short_description' => __( 'The email address of the subscriber you want to remove the tag from.', 'wp-webhooks' ),
			),
			'tag_id' => array( 
				'required' => true, 
		...
		return array(
			...
			'name'			   => __( 'Untag subscriber', 'wp-webhooks' ),
			'sentence'			   => __( 'untag a subscriber', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'returns'		   => $returns,
			'returns_code'	  => $returns_code,
			'short_description' => __( 'Untag a subscriber within "KlickTipp".', 'wp-webhooks' ),
			'description'	   => array(),
		...
		public function execute( $return_data, $response_body ){
			...
			if( ! empty( $connector ) ){
				...
				if( ! empty( $subscriber ) ){
					...
					$return_args['msg'] = __( "The subscriber was successfully untagged.", 'wp-webhooks' );
					$return_args['data']['subscriber'] = $subscriber;
				} else {
					$return_args['msg'] = __( "An error occured while untagging the subscriber.", 'wp-webhooks' );
