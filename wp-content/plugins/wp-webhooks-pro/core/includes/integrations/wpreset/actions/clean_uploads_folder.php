	...
	class WP_Webhooks_Integrations_wpreset_Actions_clean_uploads_folder {
		...
		public function get_details(){
				...
				'confirm'			=> array( 
					...
					'default_value' => 'no',
					'short_description' => __( 'Please set this value to "yes". If not set, the upload folder doesn\'t get cleaned.', 'wp-webhooks' ),
				),
...
<ol>
	...
	<li>
		...
		<?php echo __( "Contains the number of deleted files.", 'wp-webhooks' ); ?>
	</li>
</ol>
		<?php
		$parameter['do_action']['description'] = ob_get_clean();
			...
			return array(
				...
				'name'			   => __( 'Clean uploads folder', 'wp-webhooks' ),
				'sentence'			   => __( 'clean the WordPress uploads folder', 'wp-webhooks' ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => __( 'Clean your upload directory on your website using webhooks.', 'wp-webhooks' ),
				'description'	   => array(),
				'integration'	   => 'wpreset'
			);
		...
		public function execute( $return_data, $response_body ){
			...
			if( $confirm ){
				...
				$return_args['msg'] = __( "Uploads folder successfully cleaned.", 'action-clean_uploads_folder-success' );
				$return_args['data']['count'] = $count;

			} else {

				$return_args['msg'] = __( "Error: upload directory not cleared. You did not set the confirmation parameter.", 'action-clean_uploads_folder-success' );
