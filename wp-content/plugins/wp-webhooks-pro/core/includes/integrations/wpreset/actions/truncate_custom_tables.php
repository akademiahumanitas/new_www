	...
	class WP_Webhooks_Integrations_wpreset_Actions_truncate_custom_tables {
		...
		public function get_details(){
				...
				'confirm'			=> array( 
					...
					'default_value' => 'no',
					'short_description' => __( 'Please set this value to "yes". If not set, no tables get truncated.', 'wp-webhooks' )
				),
...
<ol>
	...
	<li>
		...
		<?php echo __( "Contains the number of truncated tables.", 'wp-webhooks' ); ?>
	</li>
</ol>
			<?php
			...
			$returns = array(
				...
				'data'		=> array( 'short_description' => __( '(mixed) Count of all the truncated custom tables.', 'wp-webhooks' ) ),
			...
			return array(
				...
				'name'			   => __( 'Truncate custom tables', 'wp-webhooks' ),
				'sentence'			   => __( 'truncate custom tables', 'wp-webhooks' ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => __( 'Truncate all custom tables on your website using webhooks.', 'wp-webhooks' ),
				'description'	   => array(),
				'integration'	   => 'wpreset'
			);
		...
		public function execute( $return_data, $response_body ){
			...
			if( $confirm ){
				...
				$return_args['msg'] = __( "Custom tables successfully truncated.", 'action-truncate_custom_tables-success' );
				$return_args['data']['count'] = $count;

			} else {

				$return_args['msg'] = __( "Error: Custom tables not truncated. You did not set the confirmation parameter.", 'action-truncate_custom_tables-success' );
