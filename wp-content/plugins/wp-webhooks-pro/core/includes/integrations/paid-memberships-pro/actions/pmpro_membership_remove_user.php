...
if ( ! class_exists( 'WP_Webhooks_Integrations_paid_memberships_pro_Actions_pmpro_membership_remove_user' ) ) :
	...
	public function get_details(){
		...
		$parameter = array(
			'user' => array( 'required' => true, 'short_description' => __( 'The ID or email of the user you want to remove the membership level from.', 'wp-webhooks' ) ),
			'level_id' => array( 'short_description' => __( 'The ID of the membership level you want to remove the user from. If left empty, the current membership level will be removed.', 'wp-webhooks' ) ),
		...
		$returns_code = array (
			...
			ob_start();
		?>
<?php echo __( "The do_action argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the pmpro_membership_remove_user action was fired.", 'wp-webhooks' ); ?>
<br>
...
<ol>
	<li>
		...
		<?php echo __( "The ID of the user you removed the membership from.", 'wp-webhooks' ); ?>
	</li>
	<li>
		<strong>$level_id</strong> (integer)<br>
		<?php echo __( "The ID of the membership level you removed from the user.", 'wp-webhooks' ); ?>
	</li>
	<li>
		<strong>$user_level</strong> (array)<br>
		<?php echo __( "The assigned user level before removal.", 'wp-webhooks' ); ?>
	</li>
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			return array(
				...
				'name'			  => __( 'Remove user from membership', 'wp-webhooks' ),
				'sentence'			  => __( 'remove a user from a membership', 'wp-webhooks' ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => __( 'Remove a user from a given membership within "Paid Memberships Pro".', 'wp-webhooks' ),
				'description'	   => array(),
		...
		public function execute( $return_data, $response_body ){
			...
			if( ! empty( $level_id ) && ! empty( $user_level ) && intval( $user_level->ID ) !== $level_id ){
				...
				$return_args['msg'] = __( "The user was not a member of the given membership level.", 'wp-webhooks' );
				return $return_args;
			}
			...
			if ( $level_change === true ) {
				$return_args['success'] = true;
				$return_args['msg'] = __( "The level was successfuly removed from the given user.", 'wp-webhooks' );
			} else {
				$return_args['msg'] = __( "We have been unable to remove the level from the user due to an error within Paid Memberships Pro.", 'wp-webhooks' );
			}
