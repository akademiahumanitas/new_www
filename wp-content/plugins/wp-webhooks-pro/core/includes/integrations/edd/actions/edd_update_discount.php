...
if ( ! class_exists( 'WP_Webhooks_Integrations_edd_Actions_edd_update_discount' ) ) :
	...
	class WP_Webhooks_Integrations_edd_Actions_edd_update_discount {
        ...
        public function get_details(){
            ...
            $parameter = array(
				'discount_id'       => array( 'required' => true, 'short_description' => __( '(Mixed) The discount ID or the discount code of the discount you would like to update.', 'wp-webhooks' ) ),
				'create_if_none'       => array( 
					...
					'multiple' => false,
					'default_value' => 'no',
					'short_description' => __( '(String) Set this value to "yes" in case the given discount does not exist and you want to create it. Default: no', 'wp-webhooks' ),
				),
				'code'       => array( 'short_description' => __( '(String) The dicsount code you would like to set for this dicsount. Only alphanumeric characters are allowed.', 'wp-webhooks' ) ),
				'name'       => array( 'short_description' => __( '(String) The name to identify the discount code.', 'wp-webhooks' ) ),
				'status'     => array( 'short_description' => __( '(String) The status of the discount code.', 'wp-webhooks' ) ),
			...
			$returns = array(
				...
				'data'        => array( 'short_description' => __( '(Array) Containing all of the predefined data of the webhook, as well as the discount id in case it was successfully updated (or created).', 'wp-webhooks' ) ),
			);
			...
			ob_start();
			?>
<?php echo __( "Set this argument to <strong>yes</strong> in case you want to create the discount code if it does not exist.", 'wp-webhooks' ); ?>
			<?php
...
<?php echo __( "Set this argument to <strong>yes</strong> in case you want to limit the use of this discount code to only one time per customer. Default: <strong>no</strong>", 'wp-webhooks' ); ?>
			...
			$description = array(
				'tipps' => array(
					__( "You can also use this webhook action to create a discount code in case it does not exist yet. Simply set the argument <strong>create_if_none</strong> to <strong>yes</strong>", 'wp-webhooks' ),
            ...
            return array(
                ...
                'name'              => __( 'Update discount', 'wp-webhooks' ),
                'sentence'              => __( 'update a discount', 'wp-webhooks' ),
                'parameter'         => $parameter,
                'returns'           => $returns,
                'returns_code'      => $returns_code,
                'short_description' => __( 'This webhook action allows you to update (or create) a dicsount code within Easy Digital Downloads.', 'wp-webhooks' ),
        ...
        public function execute( $return_data, $response_body ){
			...
			if( ! class_exists( 'EDD_Discount' ) ){
				$return_args['msg'] = __( 'The class EDD_Discount() is undefined. The discount code could not be updated.', 'wp-webhooks' );
			...
			if( empty( $discount_id ) || $needs_creation ){
				if( ! $create_if_none ){
					$return_args['msg'] = __( 'We could not match your given ID to a discount. No discount was updated.', 'wp-webhooks' );
			...
			if ( empty( $discount ) || empty( $discount->ID ) ) {
				if( $needs_creation ){
					...
					$return_args['msg'] = __( 'The discount code was not updated.', 'wp-webhooks' );
				}
			...
			if( $needs_creation ){
				$return_args['msg'] = __( "The discount code was successfully created.", 'action-edd_update_discount-success' );
			} else {
				$return_args['msg'] = __( "The discount code was successfully updated.", 'action-edd_update_discount-success' );
			}
