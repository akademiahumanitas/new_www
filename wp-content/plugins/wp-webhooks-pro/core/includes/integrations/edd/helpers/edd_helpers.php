	...
	class WP_Webhooks_Integrations_edd_Helpers_edd_helpers {
		...
		public function validate_payment_data( $payment_data ){
			...
			if ( empty( $payment_data ) ) {
				$response['errors'][] = __( "The payment data cannot be empty.", 'wp-webhooks' );
				$response['success'] = false;
			}

			if ( empty( $payment_data['user_info']['email'] ) ) {
				$response['errors'][] = __( "The argument user_email cannot be empty.", 'wp-webhooks' );
				$response['success'] = false;
			}
			...
			if( ! empty( $payment_data['cart_details'] ) ){
				foreach( $payment_data['cart_details'] as $item ){
					...
					if ( ! isset( $item['id'] ) ) {
						$response['errors'][] = __( "The item argument id cannot be empty. Please set it to the download id.", 'wp-webhooks' );
						$response['success'] = false;
					}

					if ( ! isset( $item['quantity'] ) ) {
						$response['errors'][] = __( "The item argument quantity cannot be empty.", 'wp-webhooks' );
						$response['success'] = false;
					}

					if ( ! isset( $item['tax'] ) ) {
						$response['errors'][] = __( "The item argument tax cannot be empty.", 'wp-webhooks' );
						$response['success'] = false;
					}
		...
		function wpwh_edd_get_order_metadata( $payment_id = 0 ) {
			...
			if( function_exists( 'edd_get_order_meta' ) ){
				$metadata = edd_get_order_meta( $payment_id );
			} else {
