  ...
  */
  class WP_Webhooks_Integrations_n8n_Triggers_n8n_http_request {
	...
	public function get_details(){
		...
		$parameter = array(
			'custom_construct' => array( 'short_description' => __( '(Mixed) The data that was sent along with the HTTP call that was made to the receivable URL from within n8n.', 'wp-webhooks' ) ),
		);

		$description = array(
			'steps' => array(
				__( 'Within the settings of this webhook trigger, copy the receivable URL.', 'wp-webhooks' ),
				__( 'Head into n8n and select the "HTTP Request" app.', 'wp-webhooks' ),
				__( 'Place the receivable URL there and send data based on your requirements.', 'wp-webhooks' ),
			),
			'tipps' => array(
				__( 'To receive data on the receivable URL, please use the "HTTP Request" app within n8n.', 'wp-webhooks' ),
		...
		return array(
			...
			'name'			  => __( 'n8n HTTP request received', 'wp-webhooks' ),
			'sentence'			  => __( 'a n8n HTTP request was received', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => sprintf( __( 'This webhook fires as soon as a HTTP request was received from the "HTTP Request" app of n8n.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() ),
