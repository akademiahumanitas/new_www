		...
		public function get_details(){
			...
			$parameter = array(
				'url'	   => array( 
					'required' => true, 
					'multiple' => true, 
					'label' => __( 'n8n "Webhook" URL', 'wp-webhooks' ), 
					'short_description' => __( '(string) The URL you want to send the data to from the "Webhooks" app from within n8n.', 'wp-webhooks' ), 
				),
				'method'	=> array( 
...
<?php echo __( "Set this argument to no to use unverified SSL connections for this URL.", 'wp-webhooks' ); ?>
		...
		$parameter['sslverify']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo __( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>n8n_send_webhook</strong> action was fired.", 'wp-webhooks' ); ?>
<br>
	...
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			return array(
				...
				'name'			  => __( 'Send data to n8n webhook app', 'wp-webhooks' ),
				'sentence'			  => __( 'send data to the n8n webhook app', 'wp-webhooks' ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => __( 'This webhook action allows you to send data to the "Webhooks" app of n8n from your WordPress website.', 'wp-webhooks' ),
				'description'	   => array(),
				'integration'	   => 'n8n',
				'premium' 			=> false,
			);
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $url ) ){
				$return_args['msg'] = __( "Please set the url argument.", 'action-n8n_send_webhook-failure' );
				return $return_args;
			}
			...
			if( $response['success'] ){
				...
				$return_args['msg'] = __( "The webhook request was sent successfully.", 'action-n8n_send_webhook-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = __( "An error occured while sending the webhook request.", 'action-n8n_send_webhook-success' );
				$return_args['data'] = $response;
			}
