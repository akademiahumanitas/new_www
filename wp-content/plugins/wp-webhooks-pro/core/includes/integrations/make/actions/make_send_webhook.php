		...
		public function get_details(){
			...
			$parameter = array(
				'url'	   => array( 
					'required' => true, 
					'multiple' => true, 
					'label' => __( 'Make (Integromat) webhooks URL', 'wp-webhooks' ),
					'short_description' => __( '(string) The URL you want to send the data to from the "Webhooks" app from within Make (Integromat)', 'wp-webhooks' ),
				),
				'method'	=> array( 
					'default_value' => 'POST', 
					'type' => 'select', 
				...
				'cookies'	   => array( 'type' => 'repeater', 'multiple' => true, 'short_description' => __( '(string) A JSON formatted string containing additional cookie data.', 'wp-webhooks' ) ),
...
<?php echo __( "Set this argument to no to use unverified SSL connections for this URL.", 'wp-webhooks' ); ?>
		...
		$parameter['sslverify']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo __( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>make_send_webhook</strong> action was fired.", 'wp-webhooks' ); ?>
<br>
	...
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			return array(
				...
				'name'			  => __( 'Send data to webhook app', 'wp-webhooks' ),
				'sentence'			  => __( 'send data to the Make (Integromat) webhook app', 'wp-webhooks' ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => __( 'This webhook action allows you to send data to a webhook request from your WordPress website to the "Webhooks" app of Make (Integromat).', 'wp-webhooks' ),
				'description'	   => array(),
				'integration'	   => 'make',
				'premium' 			=> false,
			);
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $url ) ){
				$return_args['msg'] = __( "Please set the url argument.", 'action-make_send_webhook-failure' );
				return $return_args;
			}
			...
			if( $response['success'] ){
				...
				$return_args['msg'] = __( "The webhook request was sent successfully.", 'action-make_send_webhook-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = __( "An error occured while sending the webhook request.", 'action-make_send_webhook-success' );
				$return_args['data'] = $response;
			}
