...
if ( ! class_exists( 'WP_Webhooks_Integrations_typeform_Triggers_typeform_webhook_received' ) ) :

 /**
  * Load the typeform_webhook_received trigger
  *
  * @since 6.1.4
  * @author Ironikus <info@ironikus.com>
  */
  class WP_Webhooks_Integrations_typeform_Triggers_typeform_webhook_received {

	public function get_details(){

		$parameter = array(
			'event_id' => array( 'short_description' => __( '(String) The ID of the current event within Typeform.', 'wp-webhooks' ) ),
			'event_type' => array( 'short_description' => __( '(String) The type of the event.', 'wp-webhooks' ) ),
			'form_response' => array( 'short_description' => __( '(Array) The details about this specific form request.', 'wp-webhooks' ) ),
		);
		...
		return array(
			'trigger'		   => 'typeform_webhook_received',
			'name'			  => __( 'Typeform webhook request received', 'wp-webhooks' ),
			'sentence'			  => __( 'a Typeform webhook request was received', 'wp-webhooks' ),
			'parameter'		 => $parameter,
			'settings'		  => $settings,
			'returns_code'	  => $this->get_demo( array() ),
			'short_description' => sprintf( __( 'This webhook fires as soon as a request is received from Typeform.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() ),
	...
	public function execute( $return_data, $response_body, $trigger_url_name ){
		...
		if( $trigger_url_name !== null ){
			$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'typeform_webhook_received', $trigger_url_name );
			if( ! empty( $webhooks ) ){
			...
			$webhooks = WPWHPRO()->webhook->get_hooks( 'trigger', 'typeform_webhook_received' );
		}
		...
		do_action( 'wpwhpro/webhooks/trigger_typeform_webhook_received', $return_data, $response_body, $trigger_url_name, $response_data_array );
