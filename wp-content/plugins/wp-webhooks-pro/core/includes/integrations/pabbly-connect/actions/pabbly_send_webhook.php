		...
		public function get_details(){
			...
			$parameter = array(
				'url'	   => array( 
					'required' => true, 
					'multiple' => true, 
					'label' => __( 'Pabbly Connect webhook URL', 'wp-webhooks' ), 
					'short_description' => __( '(string) The URL you want to send the data to from the "Webhook by Pabbly" app from within Pabbly Connect.', 'wp-webhooks' ), 
				),
				'method'	=> array( 
...
<?php echo __( "Set this argument to no to use unverified SSL connections for this URL.", 'wp-webhooks' ); ?>
		...
		$parameter['sslverify']['description'] = ob_get_clean();
			...
			ob_start();
		?>
<?php echo __( "The <strong>do_action</strong> argument is an advanced webhook for developers. It allows you to fire a custom WordPress hook after the <strong>pabbly_send_webhook</strong> action was fired.", 'wp-webhooks' ); ?>
<br>
	...
	<li>
		...
		$parameter['do_action']['description'] = ob_get_clean();
			...
			return array(
				...
				'name'			  => __( 'Send data to Pabbly Connect webhook app', 'wp-webhooks' ),
				'sentence'			  => __( 'send data to the Pabbly Connect webhook app', 'wp-webhooks' ),
				'parameter'		 => $parameter,
				'returns'		   => $returns,
				'returns_code'	  => $returns_code,
				'short_description' => __( 'This webhook action allows you to send data to the "Webhook by Pabbly" app of Pabbly Connect from your WordPress website.', 'wp-webhooks' ),
				'description'	   => array(),
		...
		public function execute( $return_data, $response_body ){
			...
			if( empty( $url ) ){
				$return_args['msg'] = __( "Please set the url argument.", 'action-pabbly_send_webhook-failure' );
				return $return_args;
			}
			...
			if( $response['success'] ){
				...
				$return_args['msg'] = __( "The webhook request was sent successfully.", 'action-pabbly_send_webhook-success' );
				$return_args['success'] = true;
			} else {
				$return_args['msg'] = __( "An error occured while sending the webhook request.", 'action-pabbly_send_webhook-success' );
				$return_args['data'] = $response;
			}
