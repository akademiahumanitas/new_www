...
$heading = '';
$current_content = '';
$plugin_name = __( $this->page_title, 'wp-webhooks' );
...
if( isset( $_GET['wpwhprovrs'] ) && $_GET['wpwhprovrs'] != 'home' ){
	...
	$current_content = ! empty( $current_content ) ? $current_content : __( 'Welcome to WP Webhook Pro! Currently we are not able to show you the newest informations.', 'wp-webhooks' ) ;
}
...
if( is_array( $menu_endpoints ) ){
	foreach( $menu_endpoints as $hook_name => $data ){
        ...
        if( is_array( $data ) ){
            ...
            if( isset( $data['label'] ) ){
                ...
                $title = __( 'Undefined', 'wp-webhooks' );
            }
	...
	$heading = '<li class="active"><a class="ironikus-setting-single-tab" href="?page=' . $this->page_name . '">' . __( $subs_origin['home'], 'wp-webhooks' ) . '</a></li>';
}
    ...
    <div class="wpwh-header">
        <div class="wpwh-container">
            <?php if( WPWHPRO()->whitelabel->is_active() && ! empty( WPWHPRO()->whitelabel->get_setting( 'wpwhpro_whitelabel_name' ) ) ) : ?>
                <span class="wpwh-header__logo-text"><?php echo __( WPWHPRO()->whitelabel->get_setting( 'wpwhpro_whitelabel_name' ), 'wp-webhooks' ); ?></span>
            <?php else : ?>
