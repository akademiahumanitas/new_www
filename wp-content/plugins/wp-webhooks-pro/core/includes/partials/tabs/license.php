...
if ( ! empty( $license_expires ) ) {
	...
	if ( $license_is_expired ) {
		$license_expired = __( 'Your license key has expired.', 'wp-webhooks' );
			...
			if( isset( $_POST['ironikus_activate_license'] ) && ! empty( $_POST['ironikus_activate_license'] ) || $license_activation ) {
				if( $license_status !== 'valid' ){
					...
					if ( is_wp_error( $response ) || 200 !== wp_remote_retrieve_response_code( $response ) ) {
						$message =  ( is_wp_error( $response ) && ! empty( $response->get_error_message() ) ) ? $response->get_error_message() : __( 'An error occurred, please try again.', 'wp-webhooks' );
		...
		<div class="wpwh-welcome__left col-md-6">
			<div class="wpwh-content wpwh-text-small mb-4 pb-3">
				<?php if( WPWHPRO()->whitelabel->is_active() && ! empty( WPWHPRO()->whitelabel->get_setting( 'wpwhpro_whitelabel_custom_text_license' ) ) ) : ?>
					<?php echo __( WPWHPRO()->whitelabel->get_setting( 'wpwhpro_whitelabel_custom_text_license' ), 'wp-webhooks' ); ?>
				<?php else : ?>
					<h1><?php echo __( 'Plugin License Options', 'wp-webhooks' ); ?></h1>
					<p>
					<?php echo sprintf(__( 'If you have any trouble activating your license, please check your available license slots and your subscription on your <a class="text-secondary" href="%s" target="_blank" >account page</a>.', 'wp-webhooks' ), 'https://wp-webhooks.com/'); ?>
					</p>
					<p><?php echo __( 'To activate your license, please follow the steps below:', 'wp-webhooks' ); ?></p>
					<ol>
						<li><?php echo __( 'Add the license key into the input field and press "Save license"', 'wp-webhooks' ); ?></li>
						<li><?php echo __( 'Once saved, please make sure the license is displayed as active.', 'wp-webhooks' ); ?></li>
						<li><?php echo __( 'That\'s it, you are ready to automate!', 'wp-webhooks' ); ?></li>
					</ol>
				<?php endif; ?>
			</div>
			<form method="post" action="">
				<div class="wpwh-form-field mb-4">
					<div class="d-flex align-items-start justify-content-between">
						<label class="wpwh-form-label" for="ironikus_wpwhpro_license_key"><?php echo __( 'License Key', 'wp-webhooks' ); ?></label>
							...
							<div class="wpwh-toggle wpwh-toggle--on-off">
									...
									<?php if ( ! $license_expired ) : ?>
										<label class="mb-0 mr-2 wpwh-text-small" for="ironikus_activate_license">
											<span style="color:green">
												<strong><?php echo __( 'Active', 'wp-webhooks' ); ?></strong>
											</span>
										</label>
									<?php else : ?>
										<label class="mb-0 mr-2 wpwh-text-small" for="ironikus_activate_license">
											<span style="color:red">
												<strong><?php echo __( 'Expired', 'wp-webhooks' ); ?></strong>
											</span>
										</label>
									<?php endif; ?>
								...
								<?php else : ?>
									<?php if ( ! $license_expired ) : ?>
										<label class="mb-0 mr-2 wpwh-text-small" for="ironikus_activate_license">
											<span style="color:red">
												<strong><?php echo __( 'Inactive', 'wp-webhooks' ); ?></strong>
											</span>
										</label>
									<?php else : ?>
										<label class="mb-0 mr-2 wpwh-text-small" for="ironikus_activate_license">
				...
				<div class="wpwh-form-field d-flex align-items-start">
					<label class="wpwh-form-label w-25 pb-0">
						<?php echo __( 'License Status', 'wp-webhooks' ); ?>
					</label>
					<p class="wpwh-text-small">
						<?php if ( false !== $license_status && $license_status == 'valid' ) : ?>
							<?php if ( ! $license_expired ) : ?>
								<span class="wpwh-text-success"><?php echo __( 'Active.', 'wp-webhooks' ); ?></span>
							<?php else : ?>
								<span class="wpwh-text-danger"><?php echo __( 'Expired.', 'wp-webhooks' ); ?></span>
							<?php endif; ?>
							<?php if ( ! $license_expired ) : ?>
								<?php echo sprintf( __( 'Expires on %s', 'admin-settings-license'), $license_expires ); ?>
							<?php endif; ?>
						<?php else : ?>
							<?php if ( ! $license_expired ) : ?>
								<span class="wpwh-text-danger"><?php echo __( 'Please activate your license down below.', 'wp-webhooks' ); ?></span>
							<?php else : ?>
								<span class="wpwh-text-success"><?php echo __( 'Expired.', 'wp-webhooks' ); ?></span>
							<?php endif; ?>
						<?php endif; ?>
					</p>
				</div>
				<?php echo WPWHPRO()->helpers->get_nonce_field( $license_nonce_data ); ?>
				<input type="submit" name="submit" id="submit" class="wpwh-btn wpwh-btn--secondary w-100" value="<?php echo __( 'Save License', 'wp-webhooks' ); ?>">
			</form>
		</div>
