...
<div class="wpwh-container">
  <div class="wpwh-title-area mb-4">
		...
		<?php if( WPWHPRO()->whitelabel->is_active() && ! empty( WPWHPRO()->whitelabel->get_setting( 'wpwhpro_whitelabel_custom_text_whitelist' ) ) ) : ?>
			<p><?php echo __( WPWHPRO()->whitelabel->get_setting( 'wpwhpro_whitelabel_custom_text_whitelist' ), 'wp-webhooks' ); ?></p>
		<?php else : ?>
			<p><?php echo sprintf( __( 'Down below you wil find a list of all permitted IPs that are able to access and send data to %s. You can also see the last 20 IP addresses which are not whitelisted but tried to access the endpoints. This feature also supports wildcard whitelisting - if you want to learn more about it, please click on the "Add IP address button". <br>This feature protects specifically any incoming data of webhook actions URLs, as well as the data sent to receivable trigger URLs.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() ); ?></p>
		<?php endif; ?>
  </div>

  <div class="wpwh-table-container mb-5">
    <div class="wpwh-table-header d-flex align-items-center justify-content-between">
      <h2 class="mb-0"><?php echo __( 'Allowed IPs', 'wp-webhooks' ); ?></h2>
			<button class="wpwh-btn wpwh-btn--secondary wpwh-btn--sm" data-toggle="modal" data-target="#addIpAddressModal"><?php echo __( 'Add IP address', 'wp-webhooks' ); ?></button>
    </div>
    <table class="wpwh-table">
      <thead>
        <tr>
          <th><?php echo __( 'IP', 'wp-webhooks' ); ?></th>
          <th><?php echo __( 'Action', 'wp-webhooks' ); ?></th>
        </tr>
      </thead>
      <tbody>
				<?php foreach( $whitelist as $key => $url ): ?>
					...
					<tr>
						...
						<td class="wpwh-w-auto align-middle" style="width: 120px;">
							<a class="wpwh-text-danger" href="<?php echo WPWHPRO()->helpers->built_url( WPWHPRO()->helpers->get_current_url( true, true ), array_merge( $_GET, array( 'wpwhpro_whitelist_delete' => $key, ) ) ); ?>" title="<?php echo __( 'Delete', 'wp-webhooks' ); ?>" >
								<strong><?php echo __( 'Delete', 'wp-webhooks' ); ?></strong>
							</a>
						</td>
					</tr>
				<?php endforeach; ?>
	...
	<div class="wpwh-title-area mb-4">
		<h2><?php echo __( 'Requested IP\'s', 'wp-webhooks' ); ?></h2>
		<p class="wpwh-text-small"><?php echo sprintf( __( 'Below you will find a list of all IP adresses that send requests to your %s API without being whitelisted. We always save the last 20 requests.', 'wp-webhooks' ), WPWHPRO()->settings->get_page_title() ); ?></p>
	</div>
  ...
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      <div class="modal-header">
        <h3 class="modal-title"><?php echo __( 'Whitelist IP Address', 'wp-webhooks' ); ?></h3>
        <div class="wpwh-content">
            <p class="mt-4">
            	<?php echo __( 'This feature supports wildcard whitelisting. If you want to whitelist a range of IP addresses, you can define them as <code>127.0.*</code>', 'wp-webhooks' ); ?>
            </p>
        </div>
      ...
      <form action="<?php echo $clear_form_url; ?>" method="post">
        <div class="modal-body">
          <label class="wpwh-form-label" for="ironikus_WP_Webhooks_Pro_whitelist_url"><?php echo __( 'IP Address', 'wp-webhooks' ); ?></label>
					<input class="wpwh-form-input w-100" type="text" id="ironikus_WP_Webhooks_Pro_whitelist_url" name="ironikus_WP_Webhooks_Pro_whitelist_url" placeholder="<?php echo __( '127.0.0.1', 'wp-webhooks' ); ?>" />
        </div>
        <div class="modal-footer">
					<?php echo WPWHPRO()->helpers->get_nonce_field( $whitelist_nonce_data ); ?>
					<input type="submit" name="submit" id="submit" class="wpwh-btn wpwh-btn--secondary w-100" value="<?php echo __( 'Add IP address', 'wp-webhooks' ); ?>">
        </div>
      </form>
    </div>
  </div>
